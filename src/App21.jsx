...
// ═══════════════════════════════════════════════════════
// PROPERTY BUILDER
// Project → Units (Residential/Commercial) → Category → Specs + Pricing
// ═══════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════════
// PROPERTY BUILDER MODULE
// projects → project_units → unit_sale_pricing / unit_lease_pricing
// ═══════════════════════════════════════════════════════════════

const RES_CATEGORIES  = ["Studio","1 Bed","2 Bed","3 Bed","4 Bed","5 Bed+","Villa","Townhouse","Penthouse","Duplex","Plot","Loft"];
const COMM_CATEGORIES = ["Office","Retail / Shop","Warehouse","Labour Camp","Showroom","Restaurant","Medical","Mixed Use"];
const UNIT_VIEWS    = ["Sea View","Burj View","Pool View","Garden View","Park View","City View","Golf View","Canal View","Community View","Internal"];
const UNIT_FACING   = ["North","South","East","West","North-East","North-West","South-East","South-West","Corner"];
const CONDITIONS    = ["Off-plan","Ready","Resale","Renovated"];
const FIT_OUT       = ["Shell & Core","Category A","Category B","Fully Fitted","Partly Fitted"];
const CHEQUES       = [1,2,4,6,12];
...
const STATUS_COLOR = {
  ...
  Maintenance:   { c:"#5B3FAA", bg:"#EEE8F9" },
};
...
function calcPricePerSqft(price, sqft) {
  if (price && sqft && Number(sqft) > 0) return Math.round(Number(price) / Number(sqft));
  return "";
}
function calcRentPerSqft(rent, sqft) {
  if (rent && sqft && Number(sqft) > 0) return Math.round(Number(rent) / Number(sqft));
  return "";
}
...
function PropertyBuilder({ currentUser, showToast }) {
  ...
  const [view,       setView]       = useState("browser"); // browser | all
  ...
  // modals
  const [modal, setModal] = useState(null); // "proj"|"unit"|"editunit"
  ...
  const pBlank = {
    ...
    description:"", status:"Active", launch_date:"", completion_date:"", website:""
  };
  const uBlank = {
    unit_ref:"", unit_type:"Residential", sub_type:"1 Bed", purpose:"Sale",
    floor_number:"", block_or_tower:"", view:"", facing:"",
    size_sqft:"", built_up_sqft:"", plot_sqft:"", balcony_sqft:"", terrace_sqft:"",
    bedrooms:"1", bathrooms:"1", en_suite:"0", parking_spaces:"0",
    guest_bathroom:false, powder_room:false,
    maid_room:false, maid_bathroom:false, driver_room:false,
    store_room:false, laundry_room:false, study_room:false,
    garage:false, private_pool:false, private_garden:false,
    private_beach:false, roof_terrace:false,
    furnishing:"Unfurnished", condition:"Off-plan", fit_out:"",
    handover_date:"", status:"Available", is_featured:false, notes:"",
    // sale pricing
    asking_price:"", original_price:"", price_per_sqft:"",
    service_charge_sqft:"", dld_fee_pct:"4", agency_fee_pct:"2",
    noc_fee:"", admin_fee:"", oqood_fee:"",
    expected_rent:"", gross_yield:"", net_yield:"",
    booking_pct:"10", during_construction_pct:"40", on_handover_pct:"50",
    post_handover_pct:"0", post_handover_years:"0",
    post_handover_freq:"Annual", payment_plan_notes:"",
    // lease pricing
    annual_rent:"", rent_per_sqft:"", security_deposit:"",
    agency_fee_leasing:"", municipality_tax_pct:"5",
    chiller_included:false, chiller_monthly:"",
    cheques_allowed:"4", min_lease_months:"12", max_lease_months:"24",
    notice_period_months:"3", fit_out_contribution:"",
    rent_free_months:"0", escalation_pct:""
  };
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [pr, un, sp, lp] = await Promise.all([
  ...
  const getSP      = uid => salePricing.find(s => s.unit_id === uid);
  const getLP      = uid => leasePricing.find(l => l.unit_id === uid);
  const unitCount  = pid => units.filter(u => u.project_id === pid).length;

  // ── save project ────────────────────────────────────────────────
  const saveProject = async () => {
    if (!pForm.name.trim()) { showToast("Project name is required","error"); return; }
    setSaving(true);
    const payload = {
      ...pForm,
      launch_date:     pForm.launch_date     || null,
      completion_date: pForm.completion_date || null,
    ...
    setModal(null); setPForm(pBlank);
  };

  // ── save unit (+ pricing) ───────────────────────────────────────
  const saveUnit = async (isEdit = false) => {
    if (!uForm.unit_ref.trim())  { showToast("Unit reference is required","error"); return; }
    if (!selProject && !isEdit)  { showToast("Select a project first","error"); return; }
    setSaving(true);
    try {

    const unitPayload = {
      project_id:       isEdit ? selUnit.project_id : selProject.id,
      unit_ref:         uForm.unit_ref.trim(),
      unit_type:        uForm.unit_type,
      sub_type:         uForm.sub_type,
      purpose:          uForm.purpose,
      floor_number:     n0(uForm.floor_number),
      block_or_tower:   uForm.block_or_tower || null,
      view:             uForm.view || null,
      facing:           uForm.facing || null,
      size_sqft:        n0(uForm.size_sqft),
      built_up_sqft:    n0(uForm.built_up_sqft),
      plot_sqft:        n0(uForm.plot_sqft),
      balcony_sqft:     n0(uForm.balcony_sqft),
      terrace_sqft:     n0(uForm.terrace_sqft),
      bedrooms:         n0(uForm.bedrooms) ?? 0,
      bathrooms:        n0(uForm.bathrooms) ?? 1,
      en_suite:         n0(uForm.en_suite) ?? 0,
      parking_spaces:   n0(uForm.parking_spaces) ?? 0,
      guest_bathroom:   uForm.guest_bathroom,
      powder_room:      uForm.powder_room,
      maid_room:        uForm.maid_room,
      maid_bathroom:    uForm.maid_bathroom,
      driver_room:      uForm.driver_room,
      store_room:       uForm.store_room,
      laundry_room:     uForm.laundry_room,
      study_room:       uForm.study_room,
      garage:           uForm.garage,
      private_pool:     uForm.private_pool,
      private_garden:   uForm.private_garden,
      private_beach:    uForm.private_beach,
      roof_terrace:     uForm.roof_terrace,
      furnishing:       uForm.furnishing,
      condition:        uForm.condition,
      fit_out:          uForm.fit_out || null,
      handover_date:    uForm.handover_date || null,
      status:           uForm.status,
      is_featured:      uForm.is_featured,
      notes:            uForm.notes || null,
      created_by:       currentUser.id,
    };

    let unitId;
    if (isEdit) {
      const { data, error } = await supabase.from("project_units").update(unitPayload).eq("id", selUnit.id).select().single();
      if (error) { setSaving(false); showToast(error.message,"error"); return; }
      setUnits(p => p.map(u => u.id === selUnit.id ? data : u));
      unitId = selUnit.id;
    } else {
    ...
    const pid = isEdit ? selUnit.project_id : selProject.id;
    ...
    if (uForm.purpose === "Sale" || uForm.purpose === "Both") {
      const sp = {
        unit_id:                unitId,
        project_id:             pid,
        asking_price:           n0(uForm.asking_price),
        original_price:         n0(uForm.original_price),
        price_per_sqft:         n0(uForm.price_per_sqft) || calcPricePerSqft(uForm.asking_price, uForm.size_sqft) || null,
        service_charge_sqft:    n0(uForm.service_charge_sqft),
        dld_fee_pct:            n0(uForm.dld_fee_pct) ?? 4,
        agency_fee_pct:         n0(uForm.agency_fee_pct) ?? 2,
        noc_fee:                n0(uForm.noc_fee),
        admin_fee:              n0(uForm.admin_fee),
        oqood_fee:              n0(uForm.oqood_fee),
        expected_rent:          n0(uForm.expected_rent),
        gross_yield:            n0(uForm.gross_yield),
        net_yield:              n0(uForm.net_yield),
        booking_pct:            n0(uForm.booking_pct) ?? 10,
        during_construction_pct:n0(uForm.during_construction_pct) ?? 40,
        on_handover_pct:        n0(uForm.on_handover_pct) ?? 50,
        post_handover_pct:      n0(uForm.post_handover_pct) ?? 0,
        post_handover_years:    n0(uForm.post_handover_years) ?? 0,
        post_handover_freq:     uForm.post_handover_freq,
        payment_plan_notes:     uForm.payment_plan_notes || null,
      };
      const existing = getSP(unitId);
      if (existing) {
        const { data: sd } = await supabase.from("unit_sale_pricing").update(sp).eq("id", existing.id).select().single();
        if (sd) setSalePricing(p => p.map(x => x.id === existing.id ? sd : x));
      } else {
        const { data: sd } = await supabase.from("unit_sale_pricing").insert(sp).select().single();
        if (sd) setSalePricing(p => [...p, sd]);
      }
    }
    ...
    if (uForm.purpose === "Lease" || uForm.purpose === "Both") {
      const lp = {
        unit_id:              unitId,
        project_id:           pid,
        annual_rent:          n0(uForm.annual_rent),
        rent_per_sqft:        n0(uForm.rent_per_sqft) || calcRentPerSqft(uForm.annual_rent, uForm.size_sqft) || null,
        security_deposit:     n0(uForm.security_deposit),
        agency_fee_leasing:   n0(uForm.agency_fee_leasing),
        municipality_tax_pct: n0(uForm.municipality_tax_pct) ?? 5,
        chiller_included:     uForm.chiller_included,
        chiller_monthly:      n0(uForm.chiller_monthly),
        cheques_allowed:      n0(uForm.cheques_allowed) ?? 4,
        min_lease_months:     n0(uForm.min_lease_months) ?? 12,
        max_lease_months:     n0(uForm.max_lease_months) ?? 24,
        notice_period_months: n0(uForm.notice_period_months) ?? 3,
        fit_out_contribution: n0(uForm.fit_out_contribution),
        rent_free_months:     n0(uForm.rent_free_months) ?? 0,
        escalation_pct:       n0(uForm.escalation_pct),
      };
      const existing = getLP(unitId);
      if (existing) {
        const { data: ld } = await supabase.from("unit_lease_pricing").update(lp).eq("id", existing.id).select().single();
        if (ld) setLeasePricing(p => p.map(x => x.id === existing.id ? ld : x));
      } else {
        const { data: ld } = await supabase.from("unit_lease_pricing").insert(lp).select().single();
        if (ld) setLeasePricing(p => [...p, ld]);
      }
    }

      setSaving(false);
      showToast(isEdit ? "Unit updated" : "Unit added", "success");
      setModal(null);
      setUForm(uBlank);
      if (isEdit) {
        const fresh = await supabase.from("project_units").select("*").eq("id", unitId).single();
        if (fresh.data) setSelUnit(fresh.data);
      }
    } catch(e) {
      setSaving(false);
      showToast("Error: " + (e.message || "Something went wrong. Check console."), "error");
      console.error("saveUnit error:", e);
    }
  };

  const delUnit = async (id) => {
    if (!canDel || !window.confirm("Delete this unit and all its pricing?")) return;
    await supabase.from("project_units").delete().eq("id", id);
    setUnits(p => p.filter(u => u.id !== id));
    setSalePricing(p => p.filter(s => s.unit_id !== id));
    setLeasePricing(p => p.filter(l => l.unit_id !== id));
    setSelUnit(null);
    showToast("Unit deleted","info");
  };

  const delProject = async (id) => {
    if (!canDel || !window.confirm("Delete this project and ALL its units?")) return;
    await supabase.from("projects").delete().eq("id", id);
    setProjects(p => p.filter(x => x.id !== id));
    setUnits(p => p.filter(u => u.project_id !== id));
    setSelProject(null); setSelUnit(null);
  ...
  const openEditUnit = (u) => {
    ...
    setUForm({
      ...
      en_suite:                u.en_suite ?? 0,
      parking_spaces:          u.parking_spaces ?? 0,
      size_sqft:               u.size_sqft ?? "",
      built_up_sqft:           u.built_up_sqft ?? "",
      plot_sqft:               u.plot_sqft ?? "",
      balcony_sqft:            u.balcony_sqft ?? "",
      terrace_sqft:            u.terrace_sqft ?? "",
      handover_date:           u.handover_date ?? "",
      fit_out:                 u.fit_out ?? "",
      block_or_tower:          u.block_or_tower ?? "",
      view:                    u.view ?? "",
      facing:                  u.facing ?? "",
      notes:                   u.notes ?? "",
      // sale pricing
      ...
      price_per_sqft:          sp.price_per_sqft ?? "",
      service_charge_sqft:     sp.service_charge_sqft ?? "",
      dld_fee_pct:             sp.dld_fee_pct ?? 4,
      agency_fee_pct:          sp.agency_fee_pct ?? 2,
      noc_fee:                 sp.noc_fee ?? "",
      admin_fee:               sp.admin_fee ?? "",
      oqood_fee:               sp.oqood_fee ?? "",
      expected_rent:           sp.expected_rent ?? "",
      gross_yield:             sp.gross_yield ?? "",
      net_yield:               sp.net_yield ?? "",
      booking_pct:             sp.booking_pct ?? 10,
      during_construction_pct: sp.during_construction_pct ?? 40,
      on_handover_pct:         sp.on_handover_pct ?? 50,
      post_handover_pct:       sp.post_handover_pct ?? 0,
      post_handover_years:     sp.post_handover_years ?? 0,
      post_handover_freq:      sp.post_handover_freq ?? "Annual",
      payment_plan_notes:      sp.payment_plan_notes ?? "",
      // lease pricing
      annual_rent:             lp.annual_rent ?? "",
      rent_per_sqft:           lp.rent_per_sqft ?? "",
      security_deposit:        lp.security_deposit ?? "",
      agency_fee_leasing:      lp.agency_fee_leasing ?? "",
      municipality_tax_pct:    lp.municipality_tax_pct ?? 5,
      chiller_included:        lp.chiller_included ?? false,
      chiller_monthly:         lp.chiller_monthly ?? "",
      cheques_allowed:         lp.cheques_allowed ?? 4,
      min_lease_months:        lp.min_lease_months ?? 12,
      max_lease_months:        lp.max_lease_months ?? 24,
      notice_period_months:    lp.notice_period_months ?? 3,
      fit_out_contribution:    lp.fit_out_contribution ?? "",
      rent_free_months:        lp.rent_free_months ?? 0,
      escalation_pct:          lp.escalation_pct ?? "",
    });
    setSelUnit(u);
    setModal("editunit");
  };
  ...
  const allUnits = units.filter(u => {
    ...
    const matchQ = !q || u.unit_ref.toLowerCase().includes(q)
      || proj?.name?.toLowerCase().includes(q)
      || u.sub_type?.toLowerCase().includes(q);
    const matchT = fType    === "All" || u.unit_type === fType;
    const matchS = fSub     === "All" || u.sub_type  === fSub;
    const matchP = fPurpose === "All" || u.purpose   === fPurpose || u.purpose === "Both";
    const matchSt= fStatus  === "All" || u.status    === fStatus;
    return matchQ && matchT && matchS && matchP && matchSt;
  });
  ...
  const totalSaleVal   = salePricing.reduce((s,p) => s + (p.asking_price||0), 0);

  if (loading) return <Spinner msg="Loading Property Builder…"/>;

  // ── unique sub-types for filter ─────────────────────────────────
  const allSubTypes = [...new Set(units.map(u => u.sub_type))].sort();

  const setU = (k, v) => setUForm(f => {
    const next = { ...f, [k]: v };
    // auto-calc price per sqft
    if ((k === "asking_price" || k === "size_sqft") && next.asking_price && next.size_sqft) {
      next.price_per_sqft = calcPricePerSqft(next.asking_price, next.size_sqft);
    }
    // auto-calc rent per sqft
    if ((k === "annual_rent" || k === "size_sqft") && next.annual_rent && next.size_sqft) {
      next.rent_per_sqft = calcRentPerSqft(next.annual_rent, next.size_sqft);
    }
    // auto security deposit = 5% annual rent if empty
    if (k === "annual_rent" && next.annual_rent && !next.security_deposit) {
      next.security_deposit = Math.round(Number(next.annual_rent) * 0.05);
    }
    // when type changes, reset sub_type
    if (k === "unit_type") {
      next.sub_type = v === "Residential" ? "1 Bed" : "Office";
    }
    return next;
  });
    ...
    <div className="fade-in" style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:14, alignItems:"center", flexWrap:"wrap" }}>
        <div style={{ display:"flex", gap:4 }}>
          {[["browser","🏗 Builder"],["all","📋 All Units"]].map(([id,lbl]) => (
        ...
        <div style={{ display:"flex", gap:14, marginLeft:8 }}>
          {[["Projects",projects.length,"#0B1F3A"],["Units",totalUnits,"#1A5FA8"],["Available",availableUnits,"#1A7F5A"],["Sold",soldUnits,"#B83232"],["Leased",leasedUnits,"#A06810"]].map(([l,v,c]) => (
            <div key={l} style={{ fontSize:12, color:c, fontWeight:600 }}>{l}: <span style={{ fontFamily:"'Playfair Display',serif", fontSize:15 }}>{v}</span></div>
          ))}
          {totalSaleVal > 0 && (
            <div style={{ fontSize:12, color:"#8A6200", fontWeight:600 }}>Portfolio: <span style={{ fontFamily:"'Playfair Display',serif", fontSize:15 }}>{fmtM(totalSaleVal)}</span></div>
          )}
        </div>
        <div style={{ marginLeft:"auto", display:"flex", gap:8 }}>
          {canEdit && <Btn onClick={() => { setPForm(pBlank); setModal("proj"); }}>+ New Project</Btn>}
        </div>
      </div>
      ...
      {view === "browser" && (
        <div style={{ display:"flex", gap:12, flex:1, overflow:"hidden" }}>
          ...
          {/* Column 1: Projects */}
          <div style={{ width:200, flexShrink:0, display:"flex", flexDirection:"column", overflowY:"auto" }}>
            <div style={{ fontSize:10, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.6px", marginBottom:8 }}>
            ...
            {projects.map(p => {
              ...
              return (
                <div key={p.id}
                  onClick={() => { setSelProject(p); setSelUnit(null); }}
                  style={{ padding:"10px 12px", borderRadius:10, border:`1.5px solid ${isSel?"#C9A84C":"#E2E8F0"}`, background:isSel?"#0B1F3A":"#fff", cursor:"pointer", marginBottom:6, transition:"all 0.15s" }}>
                  ...
                  <div style={{ display:"flex", gap:4, marginTop:5 }}>
                    <span style={{ fontSize:10, fontWeight:600, padding:"1px 7px", borderRadius:20, background:isSel?"rgba(201,168,76,0.2)":p.status==="Active"?"#E6F4EE":"#FDF3DC", color:isSel?"#C9A84C":p.status==="Active"?"#1A7F5A":"#A06810" }}>{p.status}</span>
                    <span style={{ fontSize:10, color:isSel?"#C9A84C44":"#A0AEC0" }}>{unitCount(p.id)} units</span>
                  </div>
                </div>
              );
            })}
          ...
          {selProject && (
            <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
              ...
              <div style={{ background:"#0B1F3A", borderRadius:10, padding:"12px 16px", marginBottom:12, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div>
                  <div style={{ fontFamily:"'Playfair Display',serif", fontSize:16, color:"#fff", fontWeight:700 }}>{selProject.name}</div>
                  <div style={{ fontSize:12, color:"#C9A84C", marginTop:2 }}>{selProject.developer}{selProject.location ? ` · 📍 ${selProject.location}` : ""}</div>
                </div>
                <div style={{ display:"flex", gap:8 }}>
                  {canDel && <Btn small variant="danger" onClick={() => delProject(selProject.id)}>Delete Project</Btn>}
                  {canEdit && <Btn small variant="gold" onClick={() => { setUForm(uBlank); setModal("unit"); }}>+ Add Unit</Btn>}
                </div>
              </div>
              ...
              <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
                <div style={{ display:"flex", gap:6, marginBottom:10 }}>
                  {[["Residential", projUnits.filter(u=>u.unit_type==="Residential").length],["Commercial", projUnits.filter(u=>u.unit_type==="Commercial").length]].map(([t,cnt]) => (
                    <button key={t} onClick={() => setUTypeTab(t)}
                      style={{ padding:"5px 16px", borderRadius:8, border:`1.5px solid ${uTypeTab===t?"#0B1F3A":"#E2E8F0"}`, background:uTypeTab===t?"#0B1F3A":"#fff", color:uTypeTab===t?"#fff":"#4A5568", fontSize:12, fontWeight:uTypeTab===t?600:400, cursor:"pointer" }}>
                      {t==="Residential"?"🏘":"🏢"} {t} ({cnt})
                    </button>
                  ))}
                </div>

                {projUnits.filter(u=>u.unit_type===uTypeTab).length === 0 && <Empty icon={uTypeTab==="Residential"?"🏘":"🏢"} msg={`No ${uTypeTab.toLowerCase()} units — click + Add Unit`}/>}
                ...
                <div style={{ flex:1, overflowY:"auto", display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))", gap:10, alignContent:"start" }}>
                      {projUnits.filter(u=>u.unit_type===uTypeTab).map(u => {
                        const sp  = getSP(u.id);
                        const lp  = getLP(u.id);
                        ...
                        return (
                          <div key={u.id} onClick={() => setSelUnit(isSel ? null : u)} className="ch"
                            ...
                            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                              ...
                              <span style={{ fontSize:10, fontWeight:600, padding:"2px 8px", borderRadius:20, background:isSel?"rgba(255,255,255,0.1)":sc.bg, color:sc.c }}>{u.status}</span>
                            </div>
                            {/* Sub-type & purpose */}
                            <div style={{ display:"flex", gap:5, marginBottom:6, flexWrap:"wrap" }}>
                              <span style={{ fontSize:10, fontWeight:600, padding:"2px 7px", borderRadius:20, background:isSel?"rgba(255,255,255,0.1)":"#F0F2F5", color:isSel?"#C9A84C":"#4A5568" }}>{u.sub_type}</span>
                            ...
                            <div style={{ fontSize:11, color:isSel?"#C9A84C88":"#A0AEC0", marginBottom:6 }}>
                              {u.bedrooms > 0 ? `${u.bedrooms} Bed` : "Studio"}
                              {u.bathrooms ? ` · ${u.bathrooms} Bath` : ""}
                              {u.size_sqft ? ` · ${Number(u.size_sqft).toLocaleString()} sqft` : ""}
                              {u.floor_number ? ` · Fl.${u.floor_number}` : ""}
                            </div>
                            {/* Facilities pills */}
                            <div style={{ display:"flex", gap:4, flexWrap:"wrap", marginBottom:6 }}>
                              {u.maid_room     && <span style={{ fontSize:9, padding:"1px 6px", borderRadius:20, background:isSel?"rgba(255,255,255,0.1)":"#EEE8F9", color:isSel?"#C9A84C":"#5B3FAA" }}>Maid</span>}
                              {u.private_pool  && <span style={{ fontSize:9, padding:"1px 6px", borderRadius:20, background:isSel?"rgba(255,255,255,0.1)":"#E6EFF9", color:isSel?"#C9A84C":"#1A5FA8" }}>Pool</span>}
                              {u.private_garden&& <span style={{ fontSize:9, padding:"1px 6px", borderRadius:20, background:isSel?"rgba(255,255,255,0.1)":"#E6F4EE", color:isSel?"#C9A84C":"#1A7F5A" }}>Garden</span>}
                              {u.parking_spaces > 0 && <span style={{ fontSize:9, padding:"1px 6px", borderRadius:20, background:isSel?"rgba(255,255,255,0.1)":"#F0F2F5", color:isSel?"#C9A84C":"#4A5568" }}>P:{u.parking_spaces}</span>}
                              {u.view && <span style={{ fontSize:9, padding:"1px 6px", borderRadius:20, background:isSel?"rgba(255,255,255,0.1)":"#FDF3DC", color:isSel?"#C9A84C":"#A06810" }}>{u.view.split(" ")[0]}</span>}
                            </div>
                            {/* Pricing */}
                            {sp?.asking_price && (u.purpose==="Sale"||u.purpose==="Both") && (
                              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:14, fontWeight:700, color:isSel?"#C9A84C":"#0B1F3A" }}>
                                {fmtAED(sp.asking_price)}
                                {sp.price_per_sqft && <span style={{ fontSize:10, fontWeight:400, color:isSel?"#C9A84C88":"#A0AEC0" }}> · AED {Number(sp.price_per_sqft).toLocaleString()}/sqft</span>}
                              </div>
                            )}
                            {lp?.annual_rent && (u.purpose==="Lease"||u.purpose==="Both") && (
                              <div style={{ fontSize:12, fontWeight:600, color:isSel?"#C9A84C":"#1A5FA8" }}>
                                🔑 {fmtAED(lp.annual_rent)}/yr
                                {lp.rent_per_sqft && <span style={{ fontSize:10, fontWeight:400, color:isSel?"#C9A84C88":"#A0AEC0" }}> · AED {Number(lp.rent_per_sqft).toLocaleString()}/sqft</span>}
                              </div>
                            )}
                          </div>
                        );
          ...
          {selUnit && (() => {
            const sp  = getSP(selUnit.id);
            const lp  = getLP(selUnit.id);
            const sc  = PB_STATUS_COLOR[selUnit.status] || {c:"#718096",bg:"#F0F2F5"};
            const proj= projects.find(p => p.id === selUnit.project_id);
            return (
              <div className="slide-in" style={{ width:320, flexShrink:0, background:"#fff", border:"1px solid #E2E8F0", borderRadius:12, overflow:"hidden", display:"flex", flexDirection:"column" }}>
                <div style={{ background:"#0B1F3A", padding:"1.25rem", position:"relative" }}>
                  ...
                  <div style={{ fontFamily:"'Playfair Display',serif", fontSize:20, color:"#C9A84C", fontWeight:700, marginBottom:4 }}>{selUnit.unit_ref}</div>
                  <div style={{ fontSize:11, color:"rgba(255,255,255,0.6)" }}>{proj?.name} · {selUnit.sub_type}</div>
                  <div style={{ display:"flex", gap:5, marginTop:8 }}>
                    ...
                    <span style={{ fontSize:10, fontWeight:600, padding:"2px 8px", borderRadius:20, background:selUnit.purpose==="Sale"?"#E6F4EE":selUnit.purpose==="Lease"?"#E6EFF9":"#FDF3DC", color:selUnit.purpose==="Sale"?"#1A7F5A":selUnit.purpose==="Lease"?"#1A5FA8":"#A06810" }}>{selUnit.purpose==="Both"?"Sale & Lease":selUnit.purpose}</span>
                    {selUnit.is_featured && <span style={{ fontSize:10, fontWeight:600, padding:"2px 8px", borderRadius:20, background:"#FDF3DC", color:"#A06810" }}>★ Featured</span>}
                  </div>
                </div>
                <div style={{ flex:1, overflowY:"auto", padding:"1rem" }}>
                  {/* Physical specs */}
                  <div style={{ fontSize:10, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.6px", marginBottom:8 }}>Specifications</div>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, background:"#FAFBFC", borderRadius:10, padding:"10px", marginBottom:12 }}>
                    <FR label="Bedrooms"    value={selUnit.bedrooms === 0 ? "Studio" : selUnit.bedrooms}/>
                    ...
                    <FR label="Total Area"  value={selUnit.total_area_sqft ? `${Number(selUnit.total_area_sqft).toLocaleString()} sqft` : "—"}/>
                    {selUnit.plot_sqft   && <FR label="Plot"     value={`${Number(selUnit.plot_sqft).toLocaleString()} sqft`}/>}
                    {selUnit.balcony_sqft&& <FR label="Balcony"  value={`${Number(selUnit.balcony_sqft).toLocaleString()} sqft`}/>}
                    <FR label="Floor"       value={selUnit.floor_number || "—"}/>
                    <FR label="Parking"     value={selUnit.parking_spaces || 0}/>
                    {selUnit.view           && <FR label="View"      value={selUnit.view}/>}
                    {selUnit.facing         && <FR label="Facing"    value={selUnit.facing}/>}
                    {selUnit.block_or_tower && <FR label="Block/Tower" value={selUnit.block_or_tower}/>}
                    <FR label="Condition"   value={selUnit.condition}/>
                    <FR label="Furnishing"  value={selUnit.furnishing}/>
                    {selUnit.handover_date  && <FR label="Handover"  value={fmtDate(selUnit.handover_date)}/>}
                  </div>
                  {/* Facilities */}
                  {(() => {
                    const facs = [];
                    if (selUnit.maid_room)      facs.push("Maid Room");
                    if (selUnit.maid_bathroom)  facs.push("Maid Bath");
                    if (selUnit.driver_room)    facs.push("Driver Room");
                    if (selUnit.store_room)     facs.push("Store Room");
                    if (selUnit.laundry_room)   facs.push("Laundry");
                    if (selUnit.study_room)     facs.push("Study");
                    if (selUnit.private_pool)   facs.push("Private Pool");
                    if (selUnit.private_garden) facs.push("Garden");
                    if (selUnit.private_beach)  facs.push("Beach Access");
                    if (selUnit.roof_terrace)   facs.push("Roof Terrace");
                    if (selUnit.garage)         facs.push("Garage");
                    if (selUnit.guest_bathroom) facs.push("Guest Bath");
                    if (selUnit.powder_room)    facs.push("Powder Room");
                    if (facs.length === 0) return null;
                    return (
                      <div style={{ marginBottom:12 }}>
                        <div style={{ fontSize:10, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.6px", marginBottom:6 }}>Facilities</div>
                        <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
                          {facs.map(f => <span key={f} style={{ fontSize:10, fontWeight:600, padding:"2px 8px", borderRadius:20, background:"#EEE8F9", color:"#5B3FAA" }}>{f}</span>)}
                        </div>
                      </div>
                    );
                  })()}
                  ...
                  {sp && (selUnit.purpose==="Sale"||selUnit.purpose==="Both") && (
                    <div style={{ background:"#FDF8EC", border:"1px solid #E8C97A", borderRadius:10, padding:"10px 12px", marginBottom:10 }}>
                      <div style={{ fontSize:10, fontWeight:700, color:"#8A6200", textTransform:"uppercase", letterSpacing:"0.6px", marginBottom:8 }}>🏷 Sale Pricing</div>
                      <div style={{ fontFamily:"'Playfair Display',serif", fontSize:18, fontWeight:700, color:"#0B1F3A", marginBottom:6 }}>{fmtAED(sp.asking_price)}</div>
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
                        {sp.price_per_sqft       && <FR label="Per sqft"       value={`AED ${Number(sp.price_per_sqft).toLocaleString()}`}/>}
                        {sp.service_charge_sqft  && <FR label="Svc Chg/sqft"  value={`AED ${sp.service_charge_sqft}/yr`}/>}
                        {sp.gross_yield          && <FR label="Gross Yield"   value={`${sp.gross_yield}%`}/>}
                        {sp.net_yield            && <FR label="Net Yield"     value={`${sp.net_yield}%`}/>}
                        <FR label="DLD Fee"       value={`${sp.dld_fee_pct||4}% = ${fmtAED(((sp.asking_price||0)*(sp.dld_fee_pct||4))/100)}`}/>
                        {sp.total_buyer_cost     && <FR label="Total Cost"    value={fmtAED(sp.total_buyer_cost)}/>}
                      </div>
                      {/* Payment plan */}
                      {(sp.booking_pct||sp.during_construction_pct||sp.on_handover_pct) && (
                        <div style={{ marginTop:8 }}>
                          <div style={{ fontSize:10, color:"#A06810", fontWeight:700, marginBottom:5 }}>Payment Plan</div>
                          <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
                            {[["Booking",sp.booking_pct],["Construction",sp.during_construction_pct],["Handover",sp.on_handover_pct],["Post-Handover",sp.post_handover_pct]].filter(([,v]) => v>0).map(([l,v]) => (
                          ...
                          {sp.post_handover_years > 0 && <div style={{ fontSize:10, color:"#1A5FA8", marginTop:4 }}>Post-handover: {sp.post_handover_years} yr{sp.post_handover_years!==1?"s":""} {sp.post_handover_freq}</div>}
                          {sp.payment_plan_notes && <div style={{ fontSize:11, color:"#4A5568", marginTop:4 }}>{sp.payment_plan_notes}</div>}
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {lp && (selUnit.purpose==="Lease"||selUnit.purpose==="Both") && (
                    <div style={{ background:"#E6EFF9", border:"1px solid #B5D4F4", borderRadius:10, padding:"10px 12px", marginBottom:10 }}>
                      <div style={{ fontSize:10, fontWeight:700, color:"#1A5FA8", textTransform:"uppercase", letterSpacing:"0.6px", marginBottom:8 }}>🔑 Lease Pricing</div>
                      <div style={{ fontFamily:"'Playfair Display',serif", fontSize:16, fontWeight:700, color:"#0B1F3A", marginBottom:6 }}>{fmtAED(lp.annual_rent)}<span style={{ fontSize:11, fontWeight:400, color:"#A0AEC0" }}>/year</span></div>
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
                        {lp.monthly_rent    && <FR label="Monthly"      value={fmtAED(lp.monthly_rent)}/>}
                        {lp.rent_per_sqft   && <FR label="Per sqft"     value={`AED ${Number(lp.rent_per_sqft).toLocaleString()}`}/>}
                        {lp.security_deposit&& <FR label="Deposit"      value={fmtAED(lp.security_deposit)}/>}
                        <FR label="Cheques"   value={`${lp.cheques_allowed} cheque${lp.cheques_allowed!==1?"s":""}`}/>
                        {lp.chiller_included ? <FR label="Chiller"    value="Included"/> : lp.chiller_monthly ? <FR label="Chiller"  value={`AED ${lp.chiller_monthly}/mo`}/> : null}
                        {lp.rent_free_months > 0 && <FR label="Rent Free" value={`${lp.rent_free_months} months`}/>}
                        {lp.escalation_pct  && <FR label="Escalation"  value={`${lp.escalation_pct}%/yr`}/>}
                      </div>
                    </div>
                  )}
                  ...
                  {canEdit && (
                    <div style={{ display:"flex", gap:8 }}>
                      <Btn full small onClick={() => openEditUnit(selUnit)}>Edit Unit</Btn>
      ...
      {view === "all" && (
        <div style={{ display:"flex", flexDirection:"column", flex:1, overflow:"hidden" }}>
          ...
          <div style={{ display:"flex", gap:8, marginBottom:10, flexWrap:"wrap" }}>
            ...
            <select value={fSub} onChange={e => setFSub(e.target.value)} style={{ width:"auto" }}>
              <option value="All">All Categories</option>
              {allSubTypes.map(s => <option key={s}>{s}</option>)}
            </select>
            <select value={fPurpose} onChange={e => setFPurpose(e.target.value)} style={{ width:"auto" }}>
              <option value="All">Sale + Lease</option>
              <option>Sale</option><option>Lease</option>
            </select>
            <select value={fStatus} onChange={e => setFStatus(e.target.value)} style={{ width:"auto" }}>
              <option value="All">All Statuses</option>
              {UNIT_STATUS.map(s => <option key={s}>{s}</option>)}
            </select>
            <span style={{ fontSize:12, color:"#A0AEC0", alignSelf:"center" }}>{allUnits.length} units</span>
          </div>
          <div style={{ flex:1, overflowY:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              <thead style={{ position:"sticky", top:0, zIndex:1 }}>
                <tr style={{ background:"#0B1F3A" }}>
                  {["Unit Ref","Project","Type","Category","Purpose","Beds","Sqft","Floor","View","Sale Price","AED/sqft","Annual Rent","Rent/sqft","Yield","Status"].map(h => (
              ...
              <tbody>
                {allUnits.map((u, i) => {
                  ...
                  return (
                    <tr key={u.id} style={{ background:i%2===0?"#fff":"#FAFBFC", borderBottom:"1px solid #F0F2F5", cursor:"pointer" }}
                      onClick={() => { setSelProject(proj); setSelUnit(u); setView("browser"); }}>
                      ...
                      <td style={{ padding:"9px 12px" }}>
                        <span style={{ fontSize:10, fontWeight:600, padding:"2px 7px", borderRadius:20, background:u.unit_type==="Residential"?"#E6F4EE":"#E6EFF9", color:u.unit_type==="Residential"?"#1A7F5A":"#1A5FA8" }}>{u.unit_type}</span>
                      </td>
                      ...
                      <td style={{ padding:"9px 12px", fontSize:11, color:"#718096", maxWidth:90, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{u.view||"—"}</td>
                      <td style={{ padding:"9px 12px", fontSize:13, fontWeight:700, color:"#0B1F3A", whiteSpace:"nowrap" }}>{sp?.asking_price?fmtAED(sp.asking_price):"—"}</td>
                      <td style={{ padding:"9px 12px", fontSize:12, color:"#4A5568" }}>{sp?.price_per_sqft?`AED ${Number(sp.price_per_sqft).toLocaleString()}`:"—"}</td>
                      <td style={{ padding:"9px 12px", fontSize:12, fontWeight:600, color:"#1A5FA8", whiteSpace:"nowrap" }}>{lp?.annual_rent?fmtAED(lp.annual_rent):"—"}</td>
                      <td style={{ padding:"9px 12px", fontSize:12, color:"#4A5568" }}>{lp?.rent_per_sqft?`AED ${Number(lp.rent_per_sqft).toLocaleString()}`:"—"}</td>
                      <td style={{ padding:"9px 12px", fontSize:12, fontWeight:600, color:"#1A7F5A" }}>{sp?.gross_yield?`${sp.gross_yield}%`:"—"}</td>
                      <td style={{ padding:"9px 12px" }}>
            ...
            {allUnits.length === 0 && <Empty icon="🏠" msg="No units match your filters"/>}
          </div>
        </div>
      )}
      ...
      {modal === "proj" && (
        <Modal title="New Project" onClose={() => setModal(null)} width={560}>
          <G2>
            <FF label="Project Name" required><input value={pForm.name} onChange={e=>setPForm(f=>({...f,name:e.target.value}))} placeholder="Emaar Beachfront"/></FF>
            <FF label="Developer"><input value={pForm.developer} onChange={e=>setPForm(f=>({...f,developer:e.target.value}))} placeholder="Emaar Properties"/></FF>
          </G2>
          <G2>
            <FF label="Location / Address"><input value={pForm.location} onChange={e=>setPForm(f=>({...f,location:e.target.value}))} placeholder="Dubai Harbour, Dubai"/></FF>
            <FF label="Community / Area"><input value={pForm.community} onChange={e=>setPForm(f=>({...f,community:e.target.value}))} placeholder="JVC, Downtown, DIFC…"/></FF>
          </G2>
          <G2>
            <FF label="Status">
          ...
          <G2>
            <FF label="Launch Date"><input type="date" value={pForm.launch_date} onChange={e=>setPForm(f=>({...f,launch_date:e.target.value}))}/></FF>
            <FF label="Completion Date"><input type="date" value={pForm.completion_date} onChange={e=>setPForm(f=>({...f,completion_date:e.target.value}))}/></FF>
          </G2>
          <FF label="Description"><textarea value={pForm.description} onChange={e=>setPForm(f=>({...f,description:e.target.value}))} rows={2} placeholder="Project highlights…"/></FF>
          <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}>
            <Btn variant="outline" onClick={() => setModal(null)}>Cancel</Btn>
            <Btn onClick={saveProject} disabled={saving}>{saving?"Saving…":"Create Project"}</Btn>
          </div>
        </Modal>
      )}

      {(modal === "unit" || modal === "editunit") && (
        <Modal title={modal==="editunit" ? `Edit — ${selUnit?.unit_ref}` : `Add New Unit to ${selProject?.name}`} onClose={() => { setModal(null); setUForm(uBlank); }} width={680}>
          <div style={{ maxHeight:"70vh", overflowY:"auto", paddingRight:4 }}>
            {/* Purpose */}
            <FF label="Purpose">
              <div style={{ display:"flex", gap:6 }}>
                {["Sale","Lease","Both"].map(p => (
                  <button key={p} onClick={() => setU("purpose",p)}
                    style={{ flex:1, padding:"9px", borderRadius:8, border:`1.5px solid ${uForm.purpose===p?"#0B1F3A":"#E2E8F0"}`, background:uForm.purpose===p?"#0B1F3A":"#fff", color:uForm.purpose===p?"#fff":"#4A5568", fontSize:13, cursor:"pointer", fontWeight:uForm.purpose===p?600:400 }}>
                    {p==="Sale"?"🏷 For Sale":p==="Lease"?"🔑 For Lease":"♻ Sale & Lease"}
                  </button>
                ))}
              </div>
            </FF>
            ...
            <div style={{ background:"#F7F9FC", border:"1px solid #E2E8F0", borderRadius:10, padding:"14px", marginBottom:14 }}>
              <div style={{ fontSize:13, fontWeight:700, color:"#0B1F3A", marginBottom:12 }}>Step 1 — What type of property is this unit?</div>
              <div style={{ display:"flex", gap:8, marginBottom:12 }}>
                {[["Residential","🏘 Residential","Apartment, Villa, Studio, Townhouse"],["Commercial","🏢 Commercial","Office, Shop, Warehouse, Showroom"]].map(([val,lbl,desc]) => (
                  <button key={val} onClick={() => setU("unit_type",val)}
                    style={{ flex:1, padding:"10px 14px", borderRadius:8, border:`2px solid ${uForm.unit_type===val?"#0B1F3A":"#E2E8F0"}`, background:uForm.unit_type===val?"#0B1F3A":"#fff", color:uForm.unit_type===val?"#fff":"#4A5568", cursor:"pointer", textAlign:"left" }}>
                    <div style={{ fontSize:14, fontWeight:700 }}>{lbl}</div>
                    <div style={{ fontSize:11, opacity:0.65, marginTop:3 }}>{desc}</div>
                  </button>
                ))}
              </div>
              <FF label="Step 2 — Category (what exactly is this unit?)">
                <select value={uForm.sub_type} onChange={e=>setU("sub_type",e.target.value)} style={{ fontSize:14, fontWeight:500 }}>
                  {(uForm.unit_type === "Residential" ? RES_CATEGORIES : COMM_CATEGORIES).map(s => <option key={s} value={s}>{s}</option>)}
                </select>
                <div style={{ fontSize:11, color:"#A0AEC0", marginTop:5, lineHeight:1.6 }}>
                  {uForm.unit_type === "Residential"
                    ? "Studio = 0 bedrooms · 1 Bed / 2 Bed / 3 Bed = apartment size · Villa = standalone house · Townhouse = 2-3 floor home · Penthouse = top floor"
                    : "Office = workspace · Retail = shop/showroom · Warehouse = storage · Labour Camp = worker housing · Showroom = display space"}
                </div>
              </FF>
            </div>
            ...
            {/* Step 3 - Reference */}
            <div style={{ background:"#F7F9FC", border:"1px solid #E2E8F0", borderRadius:10, padding:"14px", marginBottom:14 }}>
              <div style={{ fontSize:13, fontWeight:700, color:"#0B1F3A", marginBottom:12 }}>Step 3 — Unit Number & Location in Project</div>
              <G2>
                <FF label="Unit Number / Reference *">
                  <input value={uForm.unit_ref} onChange={e=>setU("unit_ref",e.target.value)} placeholder="e.g.  A-101  or  Villa-12  or  Shop-G04"/>
                  <div style={{ fontSize:11, color:"#A0AEC0", marginTop:4 }}>Your internal code. Must be unique within this project.</div>
                </FF>
                <FF label="Block / Tower / Phase (optional)">
                  <input value={uForm.block_or_tower} onChange={e=>setU("block_or_tower",e.target.value)} placeholder="e.g.  Tower A  or  Block 3"/>
                </FF>
              </G2>
            </div>

            {/* Step 4 - Rooms */}
            <div style={{ fontSize:11, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.5px", margin:"14px 0 10px" }}>Step 4 — Bedrooms, Bathrooms & Parking</div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 }}>
              <FF label="Bedrooms"><select value={uForm.bedrooms} onChange={e=>setU("bedrooms",e.target.value)}>{[0,1,2,3,4,5,6,7].map(n=><option key={n} value={n}>{n===0?"Studio (0)":n}</option>)}</select></FF>
              <FF label="Bathrooms"><select value={uForm.bathrooms} onChange={e=>setU("bathrooms",e.target.value)}>{[1,1.5,2,2.5,3,3.5,4,5,6].map(n=><option key={n} value={n}>{n}</option>)}</select></FF>
              <FF label="En-suite"><input type="number" value={uForm.en_suite} onChange={e=>setU("en_suite",e.target.value)} min="0"/></FF>
              <FF label="Parking"><input type="number" value={uForm.parking_spaces} onChange={e=>setU("parking_spaces",e.target.value)} min="0"/></FF>
            </div>

            {/* Dimensions */}
            <div style={{ fontSize:11, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.5px", margin:"14px 0 10px" }}>Step 5 — Dimensions (Size in Square Feet)</div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
              <FF label="Size (sqft)"><input type="number" value={uForm.size_sqft} onChange={e=>setU("size_sqft",e.target.value)}/></FF>
              <FF label="Built-up (sqft)"><input type="number" value={uForm.built_up_sqft} onChange={e=>setU("built_up_sqft",e.target.value)}/></FF>
              <FF label="Plot (sqft)"><input type="number" value={uForm.plot_sqft} onChange={e=>setU("plot_sqft",e.target.value)} placeholder="Villas/plots"/></FF>
              <FF label="Balcony (sqft)"><input type="number" value={uForm.balcony_sqft} onChange={e=>setU("balcony_sqft",e.target.value)}/></FF>
              <FF label="Terrace (sqft)"><input type="number" value={uForm.terrace_sqft} onChange={e=>setU("terrace_sqft",e.target.value)}/></FF>
              <FF label="Floor"><input type="number" value={uForm.floor_number} onChange={e=>setU("floor_number",e.target.value)}/></FF>
            </div>
            <G2>
              <FF label="View"><select value={uForm.view} onChange={e=>setU("view",e.target.value)}><option value="">Select…</option>{UNIT_VIEWS.map(v=><option key={v}>{v}</option>)}</select></FF>
              <FF label="Facing"><select value={uForm.facing} onChange={e=>setU("facing",e.target.value)}><option value="">Select…</option>{UNIT_FACING.map(v=><option key={v}>{v}</option>)}</select></FF>
            </G2>

            {/* Facilities */}
            <div style={{ fontSize:11, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.5px", margin:"14px 0 10px" }}>Step 6 — Facilities & Extra Rooms</div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:8, background:"#FAFBFC", borderRadius:10, padding:"12px" }}>
              {[["maid_room","Maid Room"],["maid_bathroom","Maid Bathroom"],["driver_room","Driver Room"],["store_room","Store Room"],["laundry_room","Laundry Room"],["study_room","Study Room"],["guest_bathroom","Guest Bathroom"],["powder_room","Powder Room"],["private_pool","Private Pool"],["private_garden","Private Garden"],["private_beach","Beach Access"],["roof_terrace","Roof Terrace"],["garage","Garage"]].map(([k,l]) => (
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginTop:14 }}>
              <FF label="Furnishing"><select value={uForm.furnishing} onChange={e=>setU("furnishing",e.target.value)}>{FURNISHING.map(x=><option key={x}>{x}</option>)}</select></FF>
              <FF label="Condition"><select value={uForm.condition} onChange={e=>setU("condition",e.target.value)}>{CONDITIONS.map(x=><option key={x}>{x}</option>)}</select></FF>
              <FF label="Status"><select value={uForm.status} onChange={e=>setU("status",e.target.value)}>{UNIT_STATUS.map(s=><option key={s}>{s}</option>)}</select></FF>
            </div>
            <G2>
              <FF label="Handover Date"><input type="date" value={uForm.handover_date} onChange={e=>setU("handover_date",e.target.value)}/></FF>
              <FF label="Fit-out Level"><select value={uForm.fit_out} onChange={e=>setU("fit_out",e.target.value)}><option value="">Select…</option>{FIT_OUT.map(x=><option key={x}>{x}</option>)}</select></FF>
            </G2>

            {/* SALE PRICING */}
            {(uForm.purpose==="Sale"||uForm.purpose==="Both") && (
              <>
                <div style={{ fontSize:11, fontWeight:700, color:"#8A6200", textTransform:"uppercase", letterSpacing:"0.5px", margin:"16px 0 10px" }}>🏷 Sale Pricing</div>
                <div style={{ background:"#FDF8EC", border:"1px solid #E8C97A", borderRadius:10, padding:"12px" }}>
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
                    <FF label="Asking Price (AED)"><input type="number" value={uForm.asking_price} onChange={e=>setU("asking_price",e.target.value)} placeholder="2,500,000"/></FF>
                    <FF label="Price/sqft (auto)"><input type="number" value={uForm.price_per_sqft} onChange={e=>setU("price_per_sqft",e.target.value)} placeholder="Auto-calc"/></FF>
                    <FF label="Original Price"><input type="number" value={uForm.original_price} onChange={e=>setU("original_price",e.target.value)} placeholder="Launch price"/></FF>
                    <FF label="Svc Charge/sqft/yr"><input type="number" value={uForm.service_charge_sqft} onChange={e=>setU("service_charge_sqft",e.target.value)} placeholder="AED"/></FF>
                    <FF label="Gross Yield (%)"><input type="number" value={uForm.gross_yield} onChange={e=>setU("gross_yield",e.target.value)} placeholder="7.5"/></FF>
                    <FF label="Net Yield (%)"><input type="number" value={uForm.net_yield} onChange={e=>setU("net_yield",e.target.value)}/></FF>
                    <FF label="DLD Fee (%)"><input type="number" value={uForm.dld_fee_pct} onChange={e=>setU("dld_fee_pct",e.target.value)}/></FF>
                    <FF label="Agency Fee (%)"><input type="number" value={uForm.agency_fee_pct} onChange={e=>setU("agency_fee_pct",e.target.value)}/></FF>
                    <FF label="NOC Fee (AED)"><input type="number" value={uForm.noc_fee} onChange={e=>setU("noc_fee",e.target.value)}/></FF>
                    <FF label="Oqood Fee (AED)"><input type="number" value={uForm.oqood_fee} onChange={e=>setU("oqood_fee",e.target.value)}/></FF>
                    <FF label="Admin Fee (AED)"><input type="number" value={uForm.admin_fee} onChange={e=>setU("admin_fee",e.target.value)}/></FF>
                    <FF label="Expected Rent (AED)"><input type="number" value={uForm.expected_rent} onChange={e=>setU("expected_rent",e.target.value)}/></FF>
                  </div>
                  <div style={{ fontSize:11, fontWeight:700, color:"#8A6200", margin:"12px 0 8px", textTransform:"uppercase" }}>Payment Plan (%)</div>
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8 }}>
                    {[["Booking %","booking_pct"],["Construction %","during_construction_pct"],["Handover %","on_handover_pct"],["Post-Handover %","post_handover_pct"]].map(([l,k]) => (
                      <FF key={k} label={l}><input type="number" value={uForm[k]} onChange={e=>setU(k,e.target.value)}/></FF>
                    ))}
                  </div>
                  <G2>
                    <FF label="Post-Handover Years"><input type="number" value={uForm.post_handover_years} onChange={e=>setU("post_handover_years",e.target.value)}/></FF>
                    <FF label="Post-Handover Frequency"><select value={uForm.post_handover_freq} onChange={e=>setU("post_handover_freq",e.target.value)}>{["Monthly","Quarterly","Bi-Annual","Annual"].map(x=><option key={x}>{x}</option>)}</select></FF>
                  </G2>
                  <FF label="Payment Plan Notes"><input value={uForm.payment_plan_notes} onChange={e=>setU("payment_plan_notes",e.target.value)} placeholder="e.g. 40/60 post-handover, 3 yrs interest-free"/></FF>
                  {uForm.asking_price && uForm.size_sqft && (
                    <div style={{ background:"#fff", border:"1px solid #E8C97A", borderRadius:8, padding:"8px 12px", marginTop:10, fontSize:12, color:"#8A6200" }}>
                      💡 Price/sqft: <strong>AED {Number(calcPricePerSqft(uForm.asking_price,uForm.size_sqft)).toLocaleString()}</strong>
                      &nbsp;· DLD: <strong>AED {Number((uForm.asking_price*(uForm.dld_fee_pct||4)/100)).toLocaleString()}</strong>
                      &nbsp;· Est. total: <strong>AED {Number(Number(uForm.asking_price)*(1+(Number(uForm.dld_fee_pct||4)+Number(uForm.agency_fee_pct||2))/100)).toLocaleString()}</strong>
                    </div>
                  )}
                </div>
              </>
            ...
            {(uForm.purpose==="Lease"||uForm.purpose==="Both") && (
              <>
                <div style={{ fontSize:11, fontWeight:700, color:"#1A5FA8", textTransform:"uppercase", letterSpacing:"0.5px", margin:"16px 0 10px" }}>🔑 Lease Pricing</div>
                <div style={{ background:"#E6EFF9", border:"1px solid #B5D4F4", borderRadius:10, padding:"12px" }}>
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
                    <FF label="Annual Rent (AED)"><input type="number" value={uForm.annual_rent} onChange={e=>setU("annual_rent",e.target.value)} placeholder="120,000"/></FF>
                    <FF label="Rent/sqft (auto)"><input type="number" value={uForm.rent_per_sqft} onChange={e=>setU("rent_per_sqft",e.target.value)} placeholder="Auto-calc"/></FF>
                    <FF label="Security Deposit"><input type="number" value={uForm.security_deposit} onChange={e=>setU("security_deposit",e.target.value)} placeholder="5% of rent"/></FF>
                    <FF label="Agency Fee (AED)"><input type="number" value={uForm.agency_fee_leasing} onChange={e=>setU("agency_fee_leasing",e.target.value)}/></FF>
                    <FF label="Municipality Tax %"><input type="number" value={uForm.municipality_tax_pct} onChange={e=>setU("municipality_tax_pct",e.target.value)}/></FF>
                    <FF label="Cheques Allowed"><select value={uForm.cheques_allowed} onChange={e=>setU("cheques_allowed",e.target.value)}>{CHEQUES.map(n=><option key={n} value={n}>{n===1?"Annual (1)":n===2?"Bi-Annual (2)":n===4?"Quarterly (4)":n===6?"Bi-Monthly (6)":"Monthly (12)"}</option>)}</select></FF>
                    <FF label="Min Lease (months)"><input type="number" value={uForm.min_lease_months} onChange={e=>setU("min_lease_months",e.target.value)}/></FF>
                    <FF label="Notice Period (months)"><input type="number" value={uForm.notice_period_months} onChange={e=>setU("notice_period_months",e.target.value)}/></FF>
                    <FF label="Rent-Free Months"><input type="number" value={uForm.rent_free_months} onChange={e=>setU("rent_free_months",e.target.value)}/></FF>
                    <FF label="Escalation %/yr"><input type="number" value={uForm.escalation_pct} onChange={e=>setU("escalation_pct",e.target.value)}/></FF>
                    <FF label="Fit-out Contribution (AED)"><input type="number" value={uForm.fit_out_contribution} onChange={e=>setU("fit_out_contribution",e.target.value)}/></FF>
                  </div>
                  <FF label="Chiller / District Cooling">
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      ...
                      {!uForm.chiller_included && (
                        <div style={{ flex:1 }}>
                          <input type="number" value={uForm.chiller_monthly} onChange={e=>setU("chiller_monthly",e.target.value)} placeholder="Monthly chiller cost AED" style={{ fontSize:12 }}/>
                        </div>
                      )}
                    </div>
                  </FF>
                  {uForm.annual_rent && (
                    <div style={{ background:"#fff", border:"1px solid #B5D4F4", borderRadius:8, padding:"8px 12px", marginTop:8, fontSize:12, color:"#1A5FA8" }}>
                      💡 Monthly: <strong>AED {Math.round(Number(uForm.annual_rent)/12).toLocaleString()}</strong>
                      {uForm.size_sqft && <>&nbsp;· Rent/sqft: <strong>AED {calcRentPerSqft(uForm.annual_rent,uForm.size_sqft)}</strong></>}
                      &nbsp;· Muni tax: <strong>AED {Math.round(Number(uForm.annual_rent)*(uForm.municipality_tax_pct||5)/100).toLocaleString()}/yr</strong>
                    </div>
                  )}
                </div>
              </>
            ...
            <div style={{ marginTop:14 }}>
              <FF label="Notes"><textarea value={uForm.notes} onChange={e=>setU("notes",e.target.value)} rows={2} placeholder="Additional notes…"/></FF>
              <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer" }}>
                <input type="checkbox" checked={uForm.is_featured} onChange={e=>setU("is_featured",e.target.checked)}/>
          ...
          <div style={{ display:"flex", gap:10, justifyContent:"flex-end", marginTop:14, paddingTop:14, borderTop:"1px solid #E2E8F0" }}>
            <Btn variant="outline" onClick={() => { setModal(null); setUForm(uBlank); }}>Cancel</Btn>
...
function Dashboard({ leads, properties, activities, currentUser }) {
  ...
  const DashSC = ({ label, value, sub, accent }) => (
  ...
  return (
    <div className="fade-in" style={{ display:"flex", flexDirection:"column", gap:18 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12 }}>
        <DashSC label={can(currentUser.role,"see_all")?"Active Leads":"Your Active Leads"} value={active.length} sub={`of ${visible.length} total`} accent="#0B1F3A"/>
        <DashSC label="Deals Won"        value={won.length}   sub={fmtM(wonVal)+" closed"}      accent="#1A7F5A"/>
        <DashSC label="Deals Lost"       value={lost.length}  sub="Review & re-engage"          accent="#B83232"/>
        <DashSC label="Properties Avail" value={avail}        sub={`of ${properties.length} listed`} accent="#C9A84C"/>
      </div>
