...
const RES_SUBTYPES  = ["Studio","1 Bed","2 Bed","3 Bed","4 Bed","5 Bed+","Villa","Townhouse","Penthouse","Duplex","Plot","Loft"];
const COMM_SUBTYPES = ["Office","Retail / Shop","Warehouse","Labour Camp","Showroom","Restaurant","Medical","Mixed Use"];
  ...
  return (
    <div className="fade-in" style={{ display:"flex", flexDirection:"column", height:"100%" }}>
        ...
        <div style={{ display:"flex", gap:12, flex:1, overflow:"hidden" }}>
          ...
          {selProject && (
            <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
              ...
              {/* Unit type tabs */}
              {(() => {
                const resUnits  = projUnits.filter(u => u.unit_type === "Residential");
                const commUnits = projUnits.filter(u => u.unit_type === "Commercial");
                const [uTypeTab, setUTypeTab] = useState("Residential");
                const shown = projUnits.filter(u => u.unit_type === uTypeTab);
                return (
                  <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
                    <div style={{ display:"flex", gap:6, marginBottom:10 }}>
                      {[["Residential",resUnits.length],["Commercial",commUnits.length]].map(([t,cnt]) => (
                    ...
                    {shown.length === 0 && <Empty icon={uTypeTab==="Residential"?"🏘":"🏢"} msg={`No ${uTypeTab.toLowerCase()} units — click + Add Unit`}/>}
            ...
            return (
              <div className="slide-in" style={{ width:320, flexShrink:0, background:"#fff", border:"1px solid #E2E8F0", borderRadius:12, overflow:"hidden", display:"flex", flexDirection:"column" }}>
                <div style={{ background:"#0B1F3A", padding:"1.25rem", position:"relative" }}>
                  ...
                  <div style={{ fontSize:11, color:"rgba(255,255,255,0.6)" }}>{proj?.name} · {selUnit.unit_type} · {selUnit.sub_type}</div>
          ...
          <div style={{ flex:1, overflowY:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              <thead style={{ position:"sticky", top:0, zIndex:1 }}>
                <tr style={{ background:"#0B1F3A" }}>
                  {["Unit Ref","Project","Type","Sub-type","Purpose","Beds","Sqft","Floor","View","Sale Price","AED/sqft","Annual Rent","Rent/sqft","Yield","Status"].map(h => (
      ...
      {(modal === "unit" || modal === "editunit") && (
        <Modal title={modal==="editunit" ? `Edit — ${selUnit?.unit_ref}` : `Add Unit — ${selProject?.name}`} onClose={() => { setModal(null); setUForm(uBlank); }} width={680}>
          <div style={{ maxHeight:"70vh", overflowY:"auto", paddingRight:4 }}>
            ...
            {/* Identity */}
            <div style={{ fontSize:11, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.5px", margin:"14px 0 10px" }}>Unit Identity</div>
            <G2>
              <FF label="Unit Reference" required><input value={uForm.unit_ref} onChange={e=>setU("unit_ref",e.target.value)} placeholder="A-101 / Villa-12 / Shop-G04"/></FF>
              <FF label="Block / Tower"><input value={uForm.block_or_tower} onChange={e=>setU("block_or_tower",e.target.value)} placeholder="Tower A / Block 3 / Phase 2"/></FF>
            </G2>
            <G2>
              <FF label="Unit Type">
                <div style={{ display:"flex", gap:6 }}>
                  {["Residential","Commercial"].map(t => (
                    <button key={t} onClick={() => setU("unit_type",t)}
                      style={{ flex:1, padding:"8px", borderRadius:8, border:`1.5px solid ${uForm.unit_type===t?"#0B1F3A":"#E2E8F0"}`, background:uForm.unit_type===t?"#0B1F3A":"#fff", color:uForm.unit_type===t?"#fff":"#4A5568", fontSize:12, cursor:"pointer", fontWeight:uForm.unit_type===t?600:400 }}>
              ...
              <FF label="Sub-type">
                <select value={uForm.sub_type} onChange={e=>setU("sub_type",e.target.value)}>
                  {(uForm.unit_type==="Residential" ? RES_SUBTYPES : COMM_SUBTYPES).map(s => <option key={s}>{s}</option>)}
                </select>
              </FF>
            </G2>

            {/* Bedroom config */}
            <div style={{ fontSize:11, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.5px", margin:"14px 0 10px" }}>Bedroom & Bathroom Configuration</div>
            ...
            {/* Dimensions */}
            <div style={{ fontSize:11, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.5px", margin:"14px 0 10px" }}>Dimensions</div>
            ...
            {/* Facilities */}
            <div style={{ fontSize:11, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.5px", margin:"14px 0 10px" }}>Facilities & Features</div>
