...
function Leads({leads,setLeads,properties,activities,setActivities,currentUser,users,showToast}){
  ...
  useEffect(()=>{
    const loadExtra=async()=>{
      const[m,f,u,p]=await Promise.all([
        supabase.from("meetings").select("*").order("scheduled_at"),
        supabase.from("followups").select("*").order("due_at"),
        supabase.from("units").select("id,unit_number,base_price,status,building_id,project_id,bedrooms,view,size_sqft,price_per_sqft,payment_plan").order("unit_number"),
        supabase.from("projects").select("id,name").order("name"),
      ]);
      setMeetings(m.data||[]);setFollowups(f.data||[]);setUnits(u.data||[]);setProjects(p.data||[]);
    };
    loadExtra();
  },[]);
...
const TABS=[
  ...
  {id:"properties", label:"Property Master", icon:"🏙", roles:["admin","manager","agent","viewer"]},
...
const SUBTITLES={
  ...
  properties:"Project → Category → Building → Unit master database",
  pipeline:"Drag deals across stages — gate checks enforced",
  activity:"Every call, email, WhatsApp, meeting — logged",
...
function PropertyBuilder({currentUser,showToast}) {
    ...
    <Modal title={isEdit?`Edit — ${selUnit?.unit_ref}`:`Add Unit — ${selProj?.name}`} onClose={()=>{setModal(null);setUForm(uBlank);}} width={660}>
      <div style={{maxHeight:"72vh",overflowY:"auto",paddingRight:4}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:12}}>
          {[["Bedrooms","bedrooms",[0,1,2,3,4,5,6,7],"n",n=>n===0?"Studio":n],["Bathrooms","bathrooms",[1,1.5,2,2.5,3,3.5,4,5],"n",n=>n],["Parking","parking_spaces",null,"i",null],["Floor","floor_number",null,"i",null]].map(([l,k,opts,t])=>(
            <div key={k}>
              <label style={{fontSize:11,fontWeight:600,color:"#4A5568",textTransform:"uppercase",letterSpacing:".5px",display:"block",marginBottom:5}}>{l}</label>
              {opts?<select value={uForm[k]} onChange={e=>setU(k,e.target.value)}>{opts.map(n=><option key={n} value={n}>{k==="bedrooms"&&n===0?"Studio":n}</option>)}</select>:<input type="number" value={uForm[k]} onChange={e=>setU(k,e.target.value)} min="0"/>}
            </div>
          ))}
        </div>
        {/* Sizes */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:12}}>
          {[["Size (sqft)","size_sqft","Price/sqft auto-calc"],["Built-up (sqft)","built_up_sqft",""],["Plot (sqft)","plot_sqft","Villas / plots"]].map(([l,k,h])=>(
            <div key={k}>
              <label style={{fontSize:11,fontWeight:600,color:"#4A5568",textTransform:"uppercase",letterSpacing:".5px",display:"block",marginBottom:5}}>{l}</label>
              <input type="number" value={uForm[k]} onChange={e=>setU(k,e.target.value)} placeholder={h}/>
            </div>
          ))}
        </div>
        ...
        <div style={{background:"#FAFBFC",border:"1px solid #E2E8F0",borderRadius:10,padding:"12px",marginBottom:14}}>
          <div style={{fontSize:11,fontWeight:700,color:"#0B1F3A",textTransform:"uppercase",letterSpacing:".5px",marginBottom:10}}>Facilities & Extra Rooms</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
            {[["maid_room","🧹 Maid Room"],["maid_bathroom","🚿 Maid Bathroom"],["driver_room","🚗 Driver Room"],["store_room","📦 Store Room"],["laundry_room","👕 Laundry"],["study_room","📚 Study"],["private_pool","🏊 Private Pool"],["private_garden","🌿 Garden"],["roof_terrace","🏠 Roof Terrace"],["garage","🚙 Garage"]].map(([k,l])=>(
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>
          {[["Furnishing","furnishing",["Unfurnished","Semi-Furnished","Furnished"]],["Condition","condition",["Off-plan","Ready","Resale","Renovated"]]].map(([l,k,opts])=>(
            <div key={k}>
              <label style={{fontSize:11,fontWeight:600,color:"#4A5568",textTransform:"uppercase",letterSpacing:".5px",display:"block",marginBottom:5}}>{l}</label>
              <select value={uForm[k]} onChange={e=>setU(k,e.target.value)}>{opts.map(o=><option key={o}>{o}</option>)}</select>
            </div>
          ))}
          <div>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      ...
      {modal==="unit"&&selProj&&<UnitModal/>}
      {modal==="editunit"&&selUnit&&<UnitModal isEdit/>}
    </div>
  );
}
...
export default function App(){
  ...
  const[meetings,setMeetings]=useState([]);const[followups,setFollowups]=useState([]);
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      setDataLoading(true);
      const[l,pr,a,u,m,f]=await Promise.all([
      ...
      setLeads(l.data||[]);setProperties(pr.data||[]);setActivities(a.data||[]);setUsers(u.data||[]);setMeetings(m.data||[]);setFollowups(f.data||[]);setDiscounts(d.data||[]);
