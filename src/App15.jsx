...
// ═══════════════════════════════════════════════════════
const RESIDENTIAL_SUBTYPES = ["Villa","Apartment / Flat","Townhouse","Penthouse","Studio","Duplex","Plot"];
const COMMERCIAL_SUBTYPES  = ["Office","Retail / Shop","Warehouse","Labour Camp","Showroom","Mixed Use"];
const ALL_SUBTYPES         = [...RESIDENTIAL_SUBTYPES,...COMMERCIAL_SUBTYPES];
const VIEWS_LIST = ["Sea View","Burj View","Garden View","Pool View","City View","Golf View","Internal","Canal View","Park View","Community View"];
const FURNISHING_LIST = ["Unfurnished","Semi-Furnished","Furnished"];
const SALE_STATUSES   = ["Available","Reserved","Under Offer","Sold","Blocked"];
const LEASE_STATUSES  = ["Vacant","Occupied","Reserved","Maintenance"];
const PROJ_STATUSES   = ["Active","Upcoming","Completed","On Hold"];

const SALE_STATUS_COLOR  = { Available:{c:"#1A7F5A",bg:"#E6F4EE"}, Reserved:{c:"#A06810",bg:"#FDF3DC"}, "Under Offer":{c:"#B85C10",bg:"#FDF0E6"}, Sold:{c:"#B83232",bg:"#FAEAEA"}, Blocked:{c:"#718096",bg:"#F0F2F5"} };
const LEASE_STATUS_COLOR = { Vacant:{c:"#1A7F5A",bg:"#E6F4EE"}, Occupied:{c:"#1A5FA8",bg:"#E6EFF9"}, Reserved:{c:"#A06810",bg:"#FDF3DC"}, Maintenance:{c:"#B83232",bg:"#FAEAEA"} };
  ...
  const [loading,    setLoading]    = useState(true);
  const [view,       setView]       = useState("browser"); // browser | units
  const [selProject, setSelProject] = useState(null);
  const [selType,    setSelType]    = useState(null);
  const [selSub,     setSelSub]     = useState(null);
  const [selUnit,    setSelUnit]    = useState(null);
  const [modal,      setModal]      = useState(null); // "project"|"type"|"sub"|"unit"|"editunit"
  const [saving,     setSaving]     = useState(false);
  const [unitTab,    setUnitTab]    = useState("sales"); // sales|leasing
  ...
  const tBlank  = { category:"Residential",name:"",description:"" };
  const sBlank  = { name:"Villa",purpose:"Both",description:"" };
  const uBlank  = { unit_number:"",floor_number:"",view:"",purpose:"Sales",size_sqft:"",size_sqm:"",bedrooms:"",bathrooms:"",parking_spaces:"0",balcony:false,maid_room:false,store_room:false,furnishing:"Unfurnished",condition:"Off-plan",
    sale_price:"",price_per_sqft:"",service_charge_sqft:"",gross_yield:"",net_yield:"",dld_fee_pct:"4",agency_fee_pct:"2",
    booking_pct:"10",construction_pct:"40",handover_pct:"50",post_handover_pct:"0",post_handover_years:"0",payment_plan_notes:"",
    annual_rent:"",monthly_rent:"",rent_per_sqft:"",security_deposit:"",agency_fee_leasing:"",chiller_free:false,cheques_accepted:"1",
    sale_status:"Available",lease_status:"Vacant",handover_date:"",notes:"" };
  ...
  const load = useCallback(async () => {
    ...
    const [p,t,s,u] = await Promise.all([
      ...
      supabase.from("property_subtypes").select("*"),
      supabase.from("units").select("*").order("unit_number"),
    ]);
    setProjects(p.data||[]); setPropTypes(t.data||[]);
  ...
  // ── derived ────────────────────────────────────────────────────
  const projTypes   = selProject ? propTypes.filter(t=>t.project_id===selProject.id) : [];
  const typeSubs    = selType    ? subtypes.filter(s=>s.property_type_id===selType.id) : [];
  const subUnits    = selSub     ? units.filter(u=>u.subtype_id===selSub.id) : [];
  const projUnitCount = id => units.filter(u=>u.project_id===id).length;
  const subUnitCount  = id => units.filter(u=>u.subtype_id===id).length;
  ...
  const saveType = async () => {
    if (!tForm.name.trim()||!selProject) return;
    setSaving(true);
    const { data,error } = await supabase.from("property_types").insert({ ...tForm, project_id:selProject.id }).select().single();
    setSaving(false);
    if (error) { showToast(error.message,"error"); return; }
    setPropTypes(p=>[data,...p]); showToast("Property type created","success"); setModal(null); setTForm(tBlank);
  };

  const saveSub = async () => {
    if (!sForm.name.trim()||!selType) return;
    setSaving(true);
    const { data,error } = await supabase.from("property_subtypes").insert({ ...sForm, property_type_id:selType.id, project_id:selProject.id }).select().single();
    setSaving(false);
    if (error) { showToast(error.message,"error"); return; }
    setSubtypes(p=>[data,...p]); showToast("Sub-type created","success"); setModal(null); setSForm(sBlank);
  };

  const numOrNull = v => v===""||v===null||v===undefined ? null : Number(v);

  const saveUnit = async (editing=false) => {
    if (!uForm.unit_number.trim()) { showToast("Unit number required","error"); return; }
    setSaving(true);
    const payload = {
      unit_number:uForm.unit_number, floor_number:numOrNull(uForm.floor_number),
      view:uForm.view||null, purpose:uForm.purpose,
      size_sqft:numOrNull(uForm.size_sqft), size_sqm:numOrNull(uForm.size_sqm),
      bedrooms:numOrNull(uForm.bedrooms), bathrooms:numOrNull(uForm.bathrooms),
      parking_spaces:numOrNull(uForm.parking_spaces)||0,
      balcony:uForm.balcony, maid_room:uForm.maid_room, store_room:uForm.store_room,
      furnishing:uForm.furnishing, condition:uForm.condition,
      sale_price:numOrNull(uForm.sale_price), price_per_sqft:numOrNull(uForm.price_per_sqft),
      service_charge_sqft:numOrNull(uForm.service_charge_sqft),
      gross_yield:numOrNull(uForm.gross_yield), net_yield:numOrNull(uForm.net_yield),
      dld_fee_pct:numOrNull(uForm.dld_fee_pct)||4, agency_fee_pct:numOrNull(uForm.agency_fee_pct)||2,
      booking_pct:numOrNull(uForm.booking_pct)||10, construction_pct:numOrNull(uForm.construction_pct)||40,
      handover_pct:numOrNull(uForm.handover_pct)||50, post_handover_pct:numOrNull(uForm.post_handover_pct)||0,
      post_handover_years:numOrNull(uForm.post_handover_years)||0,
      payment_plan_notes:uForm.payment_plan_notes||null,
      annual_rent:numOrNull(uForm.annual_rent), monthly_rent:numOrNull(uForm.monthly_rent),
      rent_per_sqft:numOrNull(uForm.rent_per_sqft), security_deposit:numOrNull(uForm.security_deposit),
      agency_fee_leasing:numOrNull(uForm.agency_fee_leasing),
      chiller_free:uForm.chiller_free, cheques_accepted:numOrNull(uForm.cheques_accepted)||1,
      sale_status:uForm.sale_status, lease_status:uForm.lease_status,
      handover_date:uForm.handover_date||null, notes:uForm.notes||null,
      subtype_id:selSub.id, property_type_id:selType.id, project_id:selProject.id,
      created_by:currentUser.id,
    };
    if (editing && selUnit) {
      const { data,error } = await supabase.from("units").update(payload).eq("id",selUnit.id).select().single();
      setSaving(false);
      if (error) { showToast(error.message,"error"); return; }
      setUnits(p=>p.map(u=>u.id===selUnit.id?data:u)); setSelUnit(data);
      showToast("Unit updated","success"); setModal(null);
    } else {
      const { data,error } = await supabase.from("units").insert(payload).select().single();
      setSaving(false);
      if (error) { showToast(error.message,"error"); return; }
      setUnits(p=>[...p,data]); showToast("Unit added","success"); setModal(null); setUForm(uBlank);
    }
  };

  const delProject = async (id) => {
    if (!canDel||!window.confirm("Delete this project and everything inside it?")) return;
    await supabase.from("projects").delete().eq("id",id);
    setProjects(p=>p.filter(x=>x.id!==id)); setSelProject(null); setSelType(null); setSelSub(null); setSelUnit(null);
    showToast("Project deleted","info"); load();
  };
  const delUnit = async () => {
    if (!canDel||!selUnit||!window.confirm("Delete this unit?")) return;
    await supabase.from("units").delete().eq("id",selUnit.id);
    setUnits(p=>p.filter(u=>u.id!==selUnit.id)); setSelUnit(null); showToast("Unit deleted","info");
  };

  // ── computed unit totals ───────────────────────────────────────
  const totalSaleValue  = units.filter(u=>u.sale_price).reduce((s,u)=>s+(u.sale_price||0),0);
  const totalAvailable  = units.filter(u=>u.sale_status==="Available").length;
  const totalVacant     = units.filter(u=>u.lease_status==="Vacant").length;
  const totalRentAnnual = units.filter(u=>u.annual_rent).reduce((s,u)=>s+(u.annual_rent||0),0);

  // ── All-units filtered table ───────────────────────────────────
  const allUnitsFiltered = units.filter(u => {
    const q = searchUnit.toLowerCase();
    const matchQ = !q || u.unit_number?.toLowerCase().includes(q) || projects.find(p=>p.id===u.project_id)?.name?.toLowerCase().includes(q);
    const matchP = filterPurpose==="All" || u.purpose===filterPurpose || u.purpose==="Both";
    const matchS = filterStatus==="All" || u.sale_status===filterStatus || u.lease_status===filterStatus;
    ...
    <div className="fade-in" style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:14, alignItems:"center", flexWrap:"wrap" }}>
        <div style={{ display:"flex", gap:4 }}>
          {[["browser","🗂 Browser"],["units","📋 All Units"]].map(([id,lbl])=>(
            <button key={id} onClick={()=>setView(id)} style={{ padding:"7px 14px", borderRadius:8, border:`1.5px solid ${view===id?"#0B1F3A":"#E2E8F0"}`, background:view===id?"#0B1F3A":"#fff", color:view===id?"#fff":"#4A5568", fontSize:13, fontWeight:view===id?600:400, cursor:"pointer" }}>{lbl}</button>
          ))}
        </div>
        {/* Stats */}
        <div style={{ display:"flex", gap:12, marginLeft:12 }}>
          {[["Projects",projects.length,"#0B1F3A"],["Units",units.length,"#1A5FA8"],["Available",totalAvailable,"#1A7F5A"],["Vacant",totalVacant,"#A06810"]].map(([l,v,c])=>(
            <div key={l} style={{ fontSize:12, color:c, fontWeight:600 }}>{l}: <span style={{ fontFamily:"'Playfair Display',serif", fontSize:15 }}>{v}</span></div>
          ))}
        </div>
        <div style={{ marginLeft:"auto", display:"flex", gap:8 }}>
          {canEdit && <Btn small onClick={()=>{ setPForm(pBlank); setModal("project"); }}>+ New Project</Btn>}
        </div>
      </div>
      ...
      {view==="browser" && (
        <div style={{ display:"flex", gap:12, flex:1, overflow:"hidden" }}>
          ...
          {/* Level 1: Projects */}
          <div style={{ width:210, display:"flex", flexDirection:"column", gap:6, overflowY:"auto", flexShrink:0 }}>
            <div style={{ fontSize:10, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.6px", marginBottom:4 }}>Projects ({projects.length})</div>
            {projects.length===0 && <Empty icon="📁" msg="No projects"/>}
            {projects.map(p=>{
              const isSel = selProject?.id===p.id;
              return (
                <div key={p.id} onClick={()=>{ setSelProject(p); setSelType(null); setSelSub(null); setSelUnit(null); }}
                  style={{ padding:"10px 12px", borderRadius:10, border:`1.5px solid ${isSel?"#C9A84C":"#E2E8F0"}`, background:isSel?"#0B1F3A":"#fff", cursor:"pointer", transition:"all 0.15s" }}>
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", marginTop:5 }}>
                    <span style={{ fontSize:10, fontWeight:600, padding:"2px 7px", borderRadius:20, background:isSel?"rgba(255,255,255,0.1)":p.status==="Active"?"#E6F4EE":"#FDF3DC", color:isSel?"#C9A84C":p.status==="Active"?"#1A7F5A":"#A06810" }}>{p.status}</span>
                    <span style={{ fontSize:10, color:isSel?"#C9A84C44":"#A0AEC0" }}>{projUnitCount(p.id)} units</span>
                  </div>
                </div>
              );
            })}
          ...
          {selProject && (
            <div style={{ width:190, display:"flex", flexDirection:"column", flexShrink:0, overflowY:"auto" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                <div style={{ fontSize:10, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.6px" }}>Property Types</div>
                {canEdit && <button onClick={()=>{ setTForm(tBlank); setModal("type"); }} style={{ fontSize:11, background:"none", border:"none", color:"#C9A84C", fontWeight:700, cursor:"pointer" }}>+ Add</button>}
              </div>
              {projTypes.length===0 && <div style={{ fontSize:12, color:"#A0AEC0", textAlign:"center", padding:"1.5rem 0" }}>None yet</div>}
              {projTypes.map(t=>{
                const isSel = selType?.id===t.id;
                return (
                  <div key={t.id} onClick={()=>{ setSelType(t); setSelSub(null); setSelUnit(null); }}
                    style={{ padding:"9px 12px", borderRadius:8, border:`1px solid ${isSel?"#C9A84C":"#E2E8F0"}`, background:isSel?"#FDF8EC":"#fff", cursor:"pointer", marginBottom:5 }}>
                    <div style={{ fontSize:12, fontWeight:700, color:"#0B1F3A" }}>{t.name}</div>
                    <span style={{ fontSize:10, fontWeight:600, padding:"2px 7px", borderRadius:20, background:t.category==="Residential"?"#E6F4EE":"#E6EFF9", color:t.category==="Residential"?"#1A7F5A":"#1A5FA8" }}>{t.category}</span>
                    <div style={{ fontSize:10, color:"#A0AEC0", marginTop:3 }}>{subtypes.filter(s=>s.property_type_id===t.id).length} sub-types</div>
                  </div>
                );
              })}
            </div>
          ...
          {/* Level 3: Sub-types */}
          {selType && (
            <div style={{ width:185, display:"flex", flexDirection:"column", flexShrink:0, overflowY:"auto" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                <div style={{ fontSize:10, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.6px" }}>Sub-types</div>
                {canEdit && <button onClick={()=>{ setSForm({...sBlank, name:selType.category==="Residential"?"Villa":"Office"}); setModal("sub"); }} style={{ fontSize:11, background:"none", border:"none", color:"#C9A84C", fontWeight:700, cursor:"pointer" }}>+ Add</button>}
              </div>
              {typeSubs.length===0 && <div style={{ fontSize:12, color:"#A0AEC0", textAlign:"center", padding:"1.5rem 0" }}>None yet</div>}
              {typeSubs.map(s=>{
                const isSel = selSub?.id===s.id;
                return (
                  <div key={s.id} onClick={()=>{ setSelSub(s); setSelUnit(null); }}
                    style={{ padding:"9px 12px", borderRadius:8, border:`1px solid ${isSel?"#C9A84C":"#E2E8F0"}`, background:isSel?"#FDF8EC":"#fff", cursor:"pointer", marginBottom:5 }}>
                    <div style={{ fontSize:13, fontWeight:700, color:"#0B1F3A" }}>{s.name}</div>
                    <div style={{ display:"flex", gap:5, marginTop:4 }}>
                      <span style={{ fontSize:10, fontWeight:600, padding:"2px 7px", borderRadius:20, background:"#E6EFF9", color:"#1A5FA8" }}>{s.purpose}</span>
                      <span style={{ fontSize:10, color:"#A0AEC0" }}>{subUnitCount(s.id)} units</span>
                    </div>
                  </div>
                );
              })}
          ...
          {selSub && (
            <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                <div style={{ fontSize:12, color:"#A0AEC0" }}>{subUnits.length} unit{subUnits.length!==1?"s":""} in {selSub.name}</div>
                <div style={{ display:"flex", gap:8 }}>
                  {canDel && <Btn small variant="danger" onClick={()=>delProject(selProject.id)}>Delete Project</Btn>}
                  {canEdit && <Btn small onClick={()=>{ setUForm(uBlank); setModal("unit"); }}>+ Add Unit</Btn>}
                </div>
              </div>

              {/* Purpose tabs */}
              <div style={{ display:"flex", gap:4, marginBottom:10 }}>
                {["sales","leasing","all"].map(t=>(
                  <button key={t} onClick={()=>setUnitTab(t)} style={{ padding:"5px 14px", borderRadius:8, border:`1.5px solid ${unitTab===t?"#0B1F3A":"#E2E8F0"}`, background:unitTab===t?"#0B1F3A":"#fff", color:unitTab===t?"#fff":"#4A5568", fontSize:12, fontWeight:unitTab===t?600:400, cursor:"pointer", textTransform:"capitalize" }}>{t==="all"?"All":t==="sales"?"🏷 Sales":"🔑 Leasing"}</button>
                ))}
              </div>

              <div style={{ flex:1, overflowY:"auto", display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))", gap:10, alignContent:"start" }}>
                {subUnits.filter(u=>unitTab==="all"||u.purpose===unitTab.charAt(0).toUpperCase()+unitTab.slice(1)||u.purpose==="Both").length===0 && <Empty icon="🏠" msg="No units — add the first one"/>}
                {subUnits.filter(u=>unitTab==="all"||u.purpose===unitTab.charAt(0).toUpperCase()+unitTab.slice(1)||u.purpose==="Both").map(u=>{
                  const sc = SALE_STATUS_COLOR[u.sale_status]||{c:"#718096",bg:"#F0F2F5"};
                  const lc = LEASE_STATUS_COLOR[u.lease_status]||{c:"#718096",bg:"#F0F2F5"};
                  const isSel = selUnit?.id===u.id;
                  return (
                    <div key={u.id} onClick={()=>setSelUnit(isSel?null:u)} className="ch"
                      style={{ background:isSel?"#0B1F3A":"#fff", border:`1.5px solid ${isSel?"#C9A84C":"#E2E8F0"}`, borderRadius:12, padding:"12px 14px", cursor:"pointer" }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6 }}>
                        <div style={{ fontFamily:"'Playfair Display',serif", fontSize:16, fontWeight:700, color:isSel?"#C9A84C":"#0B1F3A" }}>#{u.unit_number}</div>
                        <div style={{ display:"flex", gap:4, flexDirection:"column", alignItems:"flex-end" }}>
                          {(u.purpose==="Sales"||u.purpose==="Both") && <span style={{ fontSize:9, fontWeight:600, padding:"2px 7px", borderRadius:20, background:isSel?"rgba(255,255,255,0.1)":sc.bg, color:sc.c }}>{u.sale_status}</span>}
                          {(u.purpose==="Leasing"||u.purpose==="Both") && <span style={{ fontSize:9, fontWeight:600, padding:"2px 7px", borderRadius:20, background:isSel?"rgba(255,255,255,0.1)":lc.bg, color:lc.c }}>{u.lease_status}</span>}
                        </div>
                      </div>
                      <div style={{ fontSize:11, color:isSel?"#C9A84C88":"#A0AEC0", marginBottom:8 }}>
                        {selSub.name}{u.bedrooms?` · ${u.bedrooms}BR`:""}
                        {u.floor_number?` · Fl.${u.floor_number}`:""}
                        {u.view?` · ${u.view}`:""}
                      </div>
                      {u.sale_price && (u.purpose==="Sales"||u.purpose==="Both") && (
                        <div style={{ fontFamily:"'Playfair Display',serif", fontSize:14, fontWeight:700, color:isSel?"#C9A84C":"#0B1F3A" }}>{fmtFull(u.sale_price)}</div>
                      )}
                      {u.annual_rent && (u.purpose==="Leasing"||u.purpose==="Both") && (
                        <div style={{ fontSize:12, fontWeight:600, color:isSel?"#C9A84C":"#1A5FA8" }}>🔑 {fmtFull(u.annual_rent)}/yr</div>
                      )}
                      {u.size_sqft && <div style={{ fontSize:10, color:isSel?"#C9A84C44":"#A0AEC0" }}>{u.size_sqft?.toLocaleString()} sqft{u.price_per_sqft?` · AED ${Math.round(u.price_per_sqft).toLocaleString()}/sqft`:""}</div>}
                    </div>
                  );
                })}
              </div>
          ...
          {selUnit && (
            <div className="slide-in" style={{ width:310, background:"#fff", border:"1px solid #E2E8F0", borderRadius:12, flexShrink:0, overflow:"hidden", display:"flex", flexDirection:"column" }}>
              <div style={{ background:"#0B1F3A", padding:"1.25rem", position:"relative" }}>
                ...
                <div style={{ fontSize:11, color:"rgba(255,255,255,0.6)", marginTop:4 }}>{selProject?.name} · {selType?.name} · {selSub?.name}</div>
                <div style={{ display:"flex", gap:5, marginTop:8 }}>
                  {(selUnit.purpose==="Sales"||selUnit.purpose==="Both") && (() => { const sc=SALE_STATUS_COLOR[selUnit.sale_status]||{c:"#718096",bg:"#F0F2F5"}; return <span style={{ fontSize:10, fontWeight:600, padding:"2px 8px", borderRadius:20, background:"rgba(255,255,255,0.1)", color:sc.c }}>{selUnit.sale_status}</span>; })()}
                  {(selUnit.purpose==="Leasing"||selUnit.purpose==="Both") && (() => { const lc=LEASE_STATUS_COLOR[selUnit.lease_status]||{c:"#718096",bg:"#F0F2F5"}; return <span style={{ fontSize:10, fontWeight:600, padding:"2px 8px", borderRadius:20, background:"rgba(255,255,255,0.1)", color:lc.c }}>{selUnit.lease_status}</span>; })()}
                  <span style={{ fontSize:10, fontWeight:600, padding:"2px 8px", borderRadius:20, background:"rgba(201,168,76,0.2)", color:"#C9A84C" }}>{selUnit.purpose}</span>
                </div>
              </div>
              <div style={{ flex:1, overflowY:"auto", padding:"1rem" }}>
                {/* Specs */}
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, background:"#FAFBFC", borderRadius:10, padding:"10px", marginBottom:12 }}>
                  {[["Floor",selUnit.floor_number||"—"],["View",selUnit.view||"—"],["Size",selUnit.size_sqft?`${selUnit.size_sqft?.toLocaleString()} sqft`:"—"],["Bedrooms",selUnit.bedrooms||"—"],["Bathrooms",selUnit.bathrooms||"—"],["Parking",selUnit.parking_spaces||0],["Furnishing",selUnit.furnishing],["Condition",selUnit.condition]].map(([l,v])=>(
                  ...
                  <div style={{ gridColumn:"1/-1", display:"flex", gap:8, marginTop:4 }}>
                    {selUnit.balcony && <span style={{ fontSize:10, background:"#E6F4EE", color:"#1A7F5A", padding:"2px 7px", borderRadius:20 }}>Balcony</span>}
                    {selUnit.maid_room && <span style={{ fontSize:10, background:"#E6EFF9", color:"#1A5FA8", padding:"2px 7px", borderRadius:20 }}>Maid Room</span>}
                    {selUnit.store_room && <span style={{ fontSize:10, background:"#EEE8F9", color:"#5B3FAA", padding:"2px 7px", borderRadius:20 }}>Store Room</span>}
                    {selUnit.chiller_free && <span style={{ fontSize:10, background:"#FDF3DC", color:"#A06810", padding:"2px 7px", borderRadius:20 }}>Chiller Free</span>}
                  </div>
                </div>

                {/* Sales pricing */}
                {(selUnit.purpose==="Sales"||selUnit.purpose==="Both") && (
                  <div style={{ background:"#FDF8EC", border:"1px solid #E8C97A", borderRadius:10, padding:"10px 12px", marginBottom:10 }}>
                    <div style={{ fontSize:10, fontWeight:700, color:"#8A6200", textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:8 }}>🏷 Sales Pricing</div>
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                      <div><div style={{ fontSize:9, color:"#A0AEC0", textTransform:"uppercase" }}>Sale Price</div><div style={{ fontSize:15, fontWeight:700, color:"#0B1F3A", fontFamily:"'Playfair Display',serif" }}>{selUnit.sale_price?fmtFull(selUnit.sale_price):"TBD"}</div></div>
                      <div><div style={{ fontSize:9, color:"#A0AEC0", textTransform:"uppercase" }}>Per Sqft</div><div style={{ fontSize:12, fontWeight:600, color:"#0B1F3A" }}>{selUnit.price_per_sqft?`AED ${Math.round(selUnit.price_per_sqft).toLocaleString()}`:"—"}</div></div>
                      <div><div style={{ fontSize:9, color:"#A0AEC0", textTransform:"uppercase" }}>Gross Yield</div><div style={{ fontSize:12, fontWeight:600, color:"#1A7F5A" }}>{selUnit.gross_yield?`${selUnit.gross_yield}%`:"—"}</div></div>
                      <div><div style={{ fontSize:9, color:"#A0AEC0", textTransform:"uppercase" }}>Svc Charge/sqft</div><div style={{ fontSize:12, fontWeight:600, color:"#0B1F3A" }}>{selUnit.service_charge_sqft?`AED ${selUnit.service_charge_sqft}`:"—"}</div></div>
                      {selUnit.sale_price && selUnit.size_sqft && <div style={{ gridColumn:"1/-1" }}><div style={{ fontSize:9, color:"#A0AEC0", textTransform:"uppercase" }}>DLD Fee ({selUnit.dld_fee_pct||4}%)</div><div style={{ fontSize:12, fontWeight:600, color:"#B83232" }}>{fmtFull((selUnit.sale_price*(selUnit.dld_fee_pct||4))/100)}</div></div>}
                    </div>
                    {/* Payment plan */}
                    <div style={{ marginTop:8, borderTop:"1px solid #E8C97A", paddingTop:8 }}>
                      <div style={{ fontSize:9, color:"#A0AEC0", textTransform:"uppercase", marginBottom:5 }}>Payment Plan</div>
                      <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                        {[["Booking",selUnit.booking_pct],["Const.",selUnit.construction_pct],["Handover",selUnit.handover_pct],["Post",selUnit.post_handover_pct]].filter(([,v])=>v>0).map(([l,v])=>(
                      ...
                      {selUnit.post_handover_years>0 && <div style={{ fontSize:10, color:"#1A5FA8", marginTop:4 }}>Post-handover over {selUnit.post_handover_years} yrs</div>}
                    </div>
                  </div>
                )}

                {/* Leasing pricing */}
                {(selUnit.purpose==="Leasing"||selUnit.purpose==="Both") && (
                  <div style={{ background:"#E6EFF9", border:"1px solid #B5D4F4", borderRadius:10, padding:"10px 12px", marginBottom:10 }}>
                    <div style={{ fontSize:10, fontWeight:700, color:"#1A5FA8", textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:8 }}>🔑 Leasing Pricing</div>
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                      <div><div style={{ fontSize:9, color:"#A0AEC0", textTransform:"uppercase" }}>Annual Rent</div><div style={{ fontSize:15, fontWeight:700, color:"#0B1F3A", fontFamily:"'Playfair Display',serif" }}>{selUnit.annual_rent?fmtFull(selUnit.annual_rent):"TBD"}</div></div>
                      <div><div style={{ fontSize:9, color:"#A0AEC0", textTransform:"uppercase" }}>Monthly</div><div style={{ fontSize:12, fontWeight:600, color:"#1A5FA8" }}>{selUnit.monthly_rent?fmtFull(selUnit.monthly_rent):selUnit.annual_rent?fmtFull(Math.round(selUnit.annual_rent/12)):"—"}</div></div>
                      <div><div style={{ fontSize:9, color:"#A0AEC0", textTransform:"uppercase" }}>Rent/sqft</div><div style={{ fontSize:12, fontWeight:600 }}>{selUnit.rent_per_sqft?`AED ${selUnit.rent_per_sqft}`:"—"}</div></div>
                      <div><div style={{ fontSize:9, color:"#A0AEC0", textTransform:"uppercase" }}>Security Dep.</div><div style={{ fontSize:12, fontWeight:600 }}>{selUnit.security_deposit?fmtFull(selUnit.security_deposit):"—"}</div></div>
                      <div><div style={{ fontSize:9, color:"#A0AEC0", textTransform:"uppercase" }}>Cheques</div><div style={{ fontSize:12, fontWeight:600 }}>{selUnit.cheques_accepted||1} cheque{selUnit.cheques_accepted!==1?"s":""}</div></div>
                    </div>
                  </div>
                )}

                {selUnit.notes && <div style={{ fontSize:12, color:"#4A5568", lineHeight:1.6, padding:"8px 10px", background:"#FAFBFC", borderRadius:8, marginBottom:10 }}>{selUnit.notes}</div>}

                {canEdit && (
                  <div style={{ display:"flex", gap:8 }}>
                    <Btn full small onClick={()=>{ setUForm({...uBlank,...selUnit,floor_number:selUnit.floor_number??'',size_sqft:selUnit.size_sqft??'',size_sqm:selUnit.size_sqm??'',bedrooms:selUnit.bedrooms??'',bathrooms:selUnit.bathrooms??'',parking_spaces:selUnit.parking_spaces??0,sale_price:selUnit.sale_price??'',price_per_sqft:selUnit.price_per_sqft??'',service_charge_sqft:selUnit.service_charge_sqft??'',gross_yield:selUnit.gross_yield??'',net_yield:selUnit.net_yield??'',dld_fee_pct:selUnit.dld_fee_pct??4,agency_fee_pct:selUnit.agency_fee_pct??2,booking_pct:selUnit.booking_pct??10,construction_pct:selUnit.construction_pct??40,handover_pct:selUnit.handover_pct??50,post_handover_pct:selUnit.post_handover_pct??0,post_handover_years:selUnit.post_handover_years??0,annual_rent:selUnit.annual_rent??'',monthly_rent:selUnit.monthly_rent??'',rent_per_sqft:selUnit.rent_per_sqft??'',security_deposit:selUnit.security_deposit??'',agency_fee_leasing:selUnit.agency_fee_leasing??'',cheques_accepted:selUnit.cheques_accepted??1,handover_date:selUnit.handover_date??''}); setModal("editunit"); }}>Edit Unit</Btn>
                    {canDel && <Btn variant="danger" small onClick={delUnit}>Del</Btn>}
                  </div>
                )}
              </div>
            </div>
      ...
      {view==="units" && (
        <div style={{ display:"flex", flexDirection:"column", flex:1, overflow:"hidden" }}>
          <div style={{ display:"flex", gap:8, marginBottom:12, flexWrap:"wrap" }}>
            <input value={searchUnit} onChange={e=>setSearchUnit(e.target.value)} placeholder="🔍 Search unit or project…" style={{ flex:1, minWidth:180 }}/>
            <select value={filterPurpose} onChange={e=>setFilterPurpose(e.target.value)} style={{ width:"auto" }}>
              <option value="All">All purposes</option><option>Sales</option><option>Leasing</option>
            </select>
            <select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} style={{ width:"auto" }}>
              <option value="All">All statuses</option>
              {[...SALE_STATUSES,...LEASE_STATUSES].filter((v,i,a)=>a.indexOf(v)===i).map(s=><option key={s}>{s}</option>)}
            </select>
            <span style={{ fontSize:12, color:"#A0AEC0", alignSelf:"center" }}>{allUnitsFiltered.length} units</span>
          </div>
          <div style={{ flex:1, overflowY:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              <thead style={{ position:"sticky", top:0, zIndex:1 }}>
                <tr style={{ background:"#0B1F3A" }}>
                  {["Unit","Project","Type","Sub-type","Purpose","Floor","View","Size (sqft)","Sale Price","AED/sqft","Annual Rent","Sale Status","Lease Status"].map(h=>(
              ...
              <tbody>
                {allUnitsFiltered.map((u,i)=>{
                  ...
                  const pt   = propTypes.find(t=>t.id===u.property_type_id);
                  const st   = subtypes.find(s=>s.id===u.subtype_id);
                  const sc   = SALE_STATUS_COLOR[u.sale_status]||{c:"#718096",bg:"#F0F2F5"};
                  const lc   = LEASE_STATUS_COLOR[u.lease_status]||{c:"#718096",bg:"#F0F2F5"};
                  return (
                    <tr key={u.id} style={{ background:i%2===0?"#fff":"#FAFBFC", borderBottom:"1px solid #F0F2F5", cursor:"pointer" }}
                      onClick={()=>{ if(proj&&pt&&st){ setSelProject(proj); setSelType(pt); setSelSub(st); setSelUnit(u); setView("browser"); }}}>
                      <td style={{ padding:"9px 12px", fontWeight:700, color:"#0B1F3A", fontSize:13 }}>#{u.unit_number}</td>
                      <td style={{ padding:"9px 12px", fontSize:12, color:"#4A5568", maxWidth:130, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{proj?.name||"—"}</td>
                      <td style={{ padding:"9px 12px", fontSize:11 }}><span style={{ fontWeight:600, padding:"2px 7px", borderRadius:20, background:pt?.category==="Residential"?"#E6F4EE":"#E6EFF9", color:pt?.category==="Residential"?"#1A7F5A":"#1A5FA8" }}>{pt?.category||"—"}</span></td>
                      <td style={{ padding:"9px 12px", fontSize:12, color:"#4A5568" }}>{st?.name||"—"}</td>
                      <td style={{ padding:"9px 12px" }}><span style={{ fontSize:10, fontWeight:600, padding:"2px 7px", borderRadius:20, background:"#F0F2F5", color:"#4A5568" }}>{u.purpose}</span></td>
                      <td style={{ padding:"9px 12px", fontSize:12, color:"#4A5568" }}>{u.floor_number||"—"}</td>
                      <td style={{ padding:"9px 12px", fontSize:11, color:"#718096", maxWidth:100, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{u.view||"—"}</td>
                      <td style={{ padding:"9px 12px", fontSize:12, color:"#4A5568" }}>{u.size_sqft?u.size_sqft.toLocaleString():"—"}</td>
                      <td style={{ padding:"9px 12px", fontSize:13, fontWeight:700, color:"#0B1F3A", fontFamily:"'Playfair Display',serif", whiteSpace:"nowrap" }}>{u.sale_price?fmtFull(u.sale_price):"—"}</td>
                      <td style={{ padding:"9px 12px", fontSize:12, color:"#4A5568" }}>{u.price_per_sqft?`AED ${Math.round(u.price_per_sqft).toLocaleString()}`:"—"}</td>
                      <td style={{ padding:"9px 12px", fontSize:12, fontWeight:600, color:"#1A5FA8", whiteSpace:"nowrap" }}>{u.annual_rent?fmtFull(u.annual_rent):"—"}</td>
                      <td style={{ padding:"9px 12px" }}><span style={{ fontSize:10, fontWeight:600, padding:"2px 8px", borderRadius:20, background:sc.bg, color:sc.c }}>{u.sale_status}</span></td>
                      <td style={{ padding:"9px 12px" }}><span style={{ fontSize:10, fontWeight:600, padding:"2px 8px", borderRadius:20, background:lc.bg, color:lc.c }}>{u.lease_status}</span></td>
                    </tr>
                  );
                })}
              </tbody>
            </table>
            {allUnitsFiltered.length===0 && <Empty icon="🏠" msg="No units match your filters"/>}
          </div>
        </div>
      )}
      ...
      {/* ── MODALS ── */}
      {modal==="project" && (
        <Modal title="New Project" onClose={()=>setModal(null)} width={560}>
          ...
          <FF label="Area / District"><input value={pForm.area} onChange={e=>setPForm(f=>({...f,area:e.target.value}))} placeholder="Dubai Marina"/></FF></G2>
          <G2><FF label="Status"><select value={pForm.status} onChange={e=>setPForm(f=>({...f,status:e.target.value}))}>{PROJ_STATUSES.map(s=><option key={s}>{s}</option>)}</select></FF>
          ...
          <FF label="Description"><textarea value={pForm.description} onChange={e=>setPForm(f=>({...f,description:e.target.value}))} rows={2} placeholder="Key features and highlights…"/></FF>
          <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}><Btn variant="outline" onClick={()=>setModal(null)}>Cancel</Btn><Btn onClick={saveProject} disabled={saving}>{saving?"Saving…":"Create Project"}</Btn></div>
        </Modal>
      )}
      {modal==="type" && selProject && (
        <Modal title={`Add Property Type to ${selProject.name}`} onClose={()=>setModal(null)} width={420}>
          <FF label="Category" required>
            <div style={{ display:"flex", gap:8 }}>
              {["Residential","Commercial"].map(c=>(
                <button key={c} onClick={()=>setTForm(f=>({...f,category:c}))} style={{ flex:1, padding:"9px", borderRadius:8, border:`1.5px solid ${tForm.category===c?"#0B1F3A":"#E2E8F0"}`, background:tForm.category===c?"#0B1F3A":"#fff", color:tForm.category===c?"#fff":"#4A5568", fontSize:13, cursor:"pointer", fontWeight:tForm.category===c?600:400 }}>{c}</button>
              ))}
            </div>
          </FF>
          <FF label="Display Name" required><input value={tForm.name} onChange={e=>setTForm(f=>({...f,name:e.target.value}))} placeholder="e.g. Residential Tower A, Commercial Podium"/></FF>
          <FF label="Description"><textarea value={tForm.description} onChange={e=>setTForm(f=>({...f,description:e.target.value}))} rows={2}/></FF>
          <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}><Btn variant="outline" onClick={()=>setModal(null)}>Cancel</Btn><Btn onClick={saveType} disabled={saving}>{saving?"Saving…":"Add Type"}</Btn></div>
        </Modal>
      )}
      {modal==="sub" && selType && (
        <Modal title={`Add Sub-type to ${selType.name}`} onClose={()=>setModal(null)} width={440}>
          <FF label="Sub-type Name" required>
            <select value={sForm.name} onChange={e=>setSForm(f=>({...f,name:e.target.value}))}>
              {(selType.category==="Residential"?RESIDENTIAL_SUBTYPES:COMMERCIAL_SUBTYPES).map(s=><option key={s}>{s}</option>)}
            </select>
          </FF>
          <FF label="Purpose">
            <div style={{ display:"flex", gap:6 }}>
              {["Sales","Leasing","Both"].map(p=>(
                <button key={p} onClick={()=>setSForm(f=>({...f,purpose:p}))} style={{ flex:1, padding:"8px", borderRadius:8, border:`1.5px solid ${sForm.purpose===p?"#0B1F3A":"#E2E8F0"}`, background:sForm.purpose===p?"#0B1F3A":"#fff", color:sForm.purpose===p?"#fff":"#4A5568", fontSize:13, cursor:"pointer", fontWeight:sForm.purpose===p?600:400 }}>{p==="Sales"?"🏷 Sales":p==="Leasing"?"🔑 Leasing":"♻ Both"}</button>
              ))}
            </div>
          </FF>
          <FF label="Description"><textarea value={sForm.description} onChange={e=>setSForm(f=>({...f,description:e.target.value}))} rows={2}/></FF>
          <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}><Btn variant="outline" onClick={()=>setModal(null)}>Cancel</Btn><Btn onClick={saveSub} disabled={saving}>{saving?"Saving…":"Add Sub-type"}</Btn></div>
        </Modal>
      )}
      {(modal==="unit"||modal==="editunit") && (
        <Modal title={modal==="editunit"?`Edit Unit #${selUnit?.unit_number}`:`Add Unit — ${selSub?.name}`} onClose={()=>setModal(null)} width={640}>
          <div style={{ fontFamily:"'Playfair Display',serif", fontSize:12, color:"#A0AEC0", marginBottom:14 }}>{selProject?.name} → {selType?.name} → {selSub?.name}</div>
          {/* Purpose */}
          <FF label="Purpose">
            <div style={{ display:"flex", gap:6, marginBottom:4 }}>
              {["Sales","Leasing","Both"].map(p=>(
                <button key={p} onClick={()=>setUForm(f=>({...f,purpose:p}))} style={{ flex:1, padding:"8px", borderRadius:8, border:`1.5px solid ${uForm.purpose===p?"#0B1F3A":"#E2E8F0"}`, background:uForm.purpose===p?"#0B1F3A":"#fff", color:uForm.purpose===p?"#fff":"#4A5568", fontSize:13, cursor:"pointer", fontWeight:uForm.purpose===p?600:400 }}>{p==="Sales"?"🏷 Sales":p==="Leasing"?"🔑 Leasing":"♻ Both"}</button>
              ))}
            </div>
          </FF>
          {/* Identity */}
          ...
          <FF label="Floor"><input type="number" value={uForm.floor_number} onChange={e=>setUForm(f=>({...f,floor_number:e.target.value}))}/></FF></G2>
          <G2><FF label="View"><select value={uForm.view} onChange={e=>setUForm(f=>({...f,view:e.target.value}))}><option value="">Select…</option>{VIEWS_LIST.map(v=><option key={v}>{v}</option>)}</select></FF>
          ...
          {/* Size */}
          <G2><FF label="Size (sqft)"><input type="number" value={uForm.size_sqft} onChange={e=>{ const s=Number(e.target.value)||0; setUForm(f=>({...f,size_sqft:e.target.value,size_sqm:s?(s*0.0929).toFixed(1):"",price_per_sqft:f.sale_price&&s?Math.round(f.sale_price/s):"",rent_per_sqft:f.annual_rent&&s?Math.round(f.annual_rent/s):""})); }}/></FF>
          ...
          <G2><FF label="Parking Spaces"><input type="number" value={uForm.parking_spaces} onChange={e=>setUForm(f=>({...f,parking_spaces:e.target.value}))}/></FF>
          <FF label="Furnishing"><select value={uForm.furnishing} onChange={e=>setUForm(f=>({...f,furnishing:e.target.value}))}>{FURNISHING_LIST.map(x=><option key={x}>{x}</option>)}</select></FF></G2>
          {/* Features */}
          <FF label="Features">
            <div style={{ display:"flex", gap:16 }}>
              {[["balcony","Balcony"],["maid_room","Maid Room"],["store_room","Store Room"],["chiller_free","Chiller Free"]].map(([k,l])=>(
                <label key={k} style={{ display:"flex", alignItems:"center", gap:6, fontSize:13, color:"#4A5568", cursor:"pointer" }}>
                  <input type="checkbox" checked={uForm[k]||false} onChange={e=>setUForm(f=>({...f,[k]:e.target.checked}))}/> {l}
                </label>
              ))}
            </div>
          </FF>
          ...
          {(uForm.purpose==="Sales"||uForm.purpose==="Both") && (
            <div style={{ background:"#FDF8EC", border:"1px solid #E8C97A", borderRadius:10, padding:"12px", marginBottom:12 }}>
              <div style={{ fontSize:11, fontWeight:700, color:"#8A6200", marginBottom:10, textTransform:"uppercase", letterSpacing:"0.5px" }}>🏷 Sales Pricing</div>
              <G2><FF label="Sale Price (AED)"><input type="number" value={uForm.sale_price} onChange={e=>{ const p=Number(e.target.value)||0; setUForm(f=>({...f,sale_price:e.target.value,price_per_sqft:f.size_sqft&&p?Math.round(p/f.size_sqft):""})); }} placeholder="2500000"/></FF>
              ...
              <G2><FF label="DLD Fee (%)"><input type="number" value={uForm.dld_fee_pct} onChange={e=>setUForm(f=>({...f,dld_fee_pct:e.target.value}))}/></FF>
              <FF label="Agency Fee (%)"><input type="number" value={uForm.agency_fee_pct} onChange={e=>setUForm(f=>({...f,agency_fee_pct:e.target.value}))}/></FF></G2>
              <div style={{ fontSize:11, fontWeight:700, color:"#8A6200", margin:"10px 0 6px", textTransform:"uppercase" }}>Payment Plan (%)</div>
              ...
              <FF label="Sale Status"><select value={uForm.sale_status} onChange={e=>setUForm(f=>({...f,sale_status:e.target.value}))}>{SALE_STATUSES.map(s=><option key={s}>{s}</option>)}</select></FF></G2>
              <FF label="Payment Plan Notes"><input value={uForm.payment_plan_notes} onChange={e=>setUForm(f=>({...f,payment_plan_notes:e.target.value}))} placeholder="e.g. 60/40 post-handover over 3 years"/></FF>
            </div>
          )}
          ...
          {(uForm.purpose==="Leasing"||uForm.purpose==="Both") && (
            <div style={{ background:"#E6EFF9", border:"1px solid #B5D4F4", borderRadius:10, padding:"12px", marginBottom:12 }}>
              <div style={{ fontSize:11, fontWeight:700, color:"#1A5FA8", marginBottom:10, textTransform:"uppercase", letterSpacing:"0.5px" }}>🔑 Leasing Pricing</div>
              <G2><FF label="Annual Rent (AED)"><input type="number" value={uForm.annual_rent} onChange={e=>{ const r=Number(e.target.value)||0; setUForm(f=>({...f,annual_rent:e.target.value,monthly_rent:r?Math.round(r/12):"",rent_per_sqft:f.size_sqft&&r?Math.round(r/f.size_sqft):""})); }} placeholder="120000"/></FF>
              <FF label="Monthly Rent (AED)"><input type="number" value={uForm.monthly_rent} onChange={e=>setUForm(f=>({...f,monthly_rent:e.target.value}))} placeholder="Auto-calculated"/></FF></G2>
              <G2><FF label="Security Deposit (AED)"><input type="number" value={uForm.security_deposit} onChange={e=>setUForm(f=>({...f,security_deposit:e.target.value}))}/></FF>
              <FF label="Cheques Accepted"><input type="number" value={uForm.cheques_accepted} onChange={e=>setUForm(f=>({...f,cheques_accepted:e.target.value}))}/></FF></G2>
              <G2><FF label="Rent/sqft"><input type="number" value={uForm.rent_per_sqft} onChange={e=>setUForm(f=>({...f,rent_per_sqft:e.target.value}))} placeholder="Auto-calculated"/></FF>
              <FF label="Lease Status"><select value={uForm.lease_status} onChange={e=>setUForm(f=>({...f,lease_status:e.target.value}))}>{LEASE_STATUSES.map(s=><option key={s}>{s}</option>)}</select></FF></G2>
            </div>
          )}

          <G2><FF label="Handover Date"><input type="date" value={uForm.handover_date} onChange={e=>setUForm(f=>({...f,handover_date:e.target.value}))}/></FF></G2>
          <FF label="Notes"><textarea value={uForm.notes} onChange={e=>setUForm(f=>({...f,notes:e.target.value}))} rows={2} placeholder="Additional details…"/></FF>
          <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}>
            <Btn variant="outline" onClick={()=>setModal(null)}>Cancel</Btn>
            <Btn onClick={()=>saveUnit(modal==="editunit")} disabled={saving}>{saving?"Saving…":modal==="editunit"?"Save Changes":"Add Unit"}</Btn>
          </div>
        </Modal>
      )}
    </div>
...
// ═══════════════════════════════════════════════════════
// PROPERTY BUILDER
// Project → Property Type → Sub-type → Unit
// Purpose per unit: Sales | Leasing | Both
// ═══════════════════════════════════════════════════════

const COMM_SUBTYPES = ["Office","Retail / Shop","Warehouse","Labour Camp","Showroom","Mixed Use"];
const RESI_SUBTYPES = ["Villa","Apartment / Flat","Townhouse","Penthouse","Studio","Duplex","Plot"];
const UNIT_VIEWS    = ["Sea View","Burj View","Garden View","Pool View","City View","Golf View","Canal View","Internal","Corner"];
const UNIT_FACING   = ["North","South","East","West","North-East","North-West","South-East","South-West","Corner"];
const CHEQUE_OPTS   = [1,2,4,6,12];

const PB_STATUS_COLOR = {
  Available:     { c:"#1A7F5A", bg:"#E6F4EE" },
  Reserved:      { c:"#A06810", bg:"#FDF3DC" },
  "Under Offer": { c:"#B85C10", bg:"#FDF0E6" },
  Sold:          { c:"#B83232", bg:"#FAEAEA" },
  Rented:        { c:"#1A5FA8", bg:"#E6EFF9" },
  Blocked:       { c:"#718096", bg:"#F0F2F5" },
  "Off Market":  { c:"#718096", bg:"#F0F2F5" },
};
...
// ═══════════════════════════════════════════════════════
// LEASING MODULE
// Tenants · Leases · Rent · Renewals · Maintenance
// ═══════════════════════════════════════════════════════
) {
  ...
  const [pbUnits,      setPbUnits]     = useState([]);
  ...
  const [selLease,        setSelLease]         = useState(null);

  // Forms
  const tBlank = { full_name:"",email:"",phone:"",whatsapp:"",nationality:"",passport_no:"",passport_expiry:"",visa_no:"",visa_expiry:"",emirates_id:"",emirates_id_expiry:"",trade_license_no:"",company_name:"",tenant_type:"Individual",status:"Active",notes:"" };
  const lBlank = { tenant_id:"",unit_id:"",lease_type:"Residential",contract_no:"",start_date:"",end_date:"",notice_period_days:"90",annual_rent:"",security_deposit:"",no_of_cheques:"4",agency_commission:"",ejari_no:"",rera_no:"",notes:"",status:"Active" };
  const pBlank = { lease_id:"",cheque_no:"",bank_name:"",amount:"",due_date:"",payment_method:"Cheque",period_from:"",period_to:"",status:"Pending",notes:"" };
  const mBlank = { tenant_id:"",unit_id:"",title:"",description:"",category:"Plumbing",priority:"Medium",contractor_name:"",contractor_phone:"",estimated_cost:"",charged_to:"Owner",status:"Open",notes:"" };
  const [tForm, setTForm] = useState(tBlank);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [te,le,pa,ma,u,p] = await Promise.all([
      supabase.from("tenants").select("*").order("created_at",{ascending:false}),
      supabase.from("leases").select("*").order("created_at",{ascending:false}),
      supabase.from("rent_payments").select("*").order("due_date",{ascending:true}),
      supabase.from("maintenance").select("*").order("created_at",{ascending:false}),
      supabase.from("pb_units").select("id,unit_ref,project_id,annual_rent,sale_price").order("unit_ref"),
      supabase.from("pb_projects").select("id,name"),
    ]);
    setTenants(te.data||[]); setLeases(le.data||[]);
    setPayments(pa.data||[]); setMaintenance(ma.data||[]);
    setPbUnits(u.data||[]); setPbProjects(p.data||[]);
    setLoading(false);
  },[]);
  useEffect(()=>{ load(); },[load]);

  const saveTenant = async () => {
    if (!tForm.full_name.trim()) { showToast("Full name required","error"); return; }
    const payload = { ...tForm, passport_expiry:tForm.passport_expiry||null, visa_expiry:tForm.visa_expiry||null, emirates_id_expiry:tForm.emirates_id_expiry||null, created_by:currentUser.id };
    const { data,error } = await supabase.from("tenants").insert(payload).select().single();
    if (error) { showToast(error.message,"error"); return; }
    setTenants(p=>[data,...p]); showToast("Tenant created","success"); setShowAddTenant(false); setTForm(tBlank);
  };

  const saveLease = async () => {
    if (!lForm.tenant_id||!lForm.unit_id||!lForm.start_date||!lForm.end_date||!lForm.annual_rent) { showToast("Tenant, unit, dates and rent required","error"); return; }
    const ar = Number(lForm.annual_rent);
    const years = (new Date(lForm.end_date)-new Date(lForm.start_date))/(365.25*24*3600*1000);
    const payload = { ...lForm, annual_rent:ar, security_deposit:Number(lForm.security_deposit)||null, agency_commission:Number(lForm.agency_commission)||null, no_of_cheques:Number(lForm.no_of_cheques)||4, notice_period_days:Number(lForm.notice_period_days)||90, municipality_fee: ar*0.05, total_contract_value: ar*years, created_by:currentUser.id };
    const { data,error } = await supabase.from("leases").insert(payload).select().single();
    if (error) { showToast(error.message,"error"); return; }
    // Update unit status to Rented
    await supabase.from("pb_units").update({ status:"Rented" }).eq("id",lForm.unit_id);
    setLeases(p=>[data,...p]); showToast("Lease created","success"); setShowAddLease(false); setLForm(lBlank);
  };
  ...
  const savePayment = async () => {
    ...
    const payload = { ...pForm, amount:Number(pForm.amount), tenant_id:lease?.tenant_id, unit_id:lease?.unit_id, recorded_by:currentUser.id };
    const { data,error } = await supabase.from("rent_payments").insert(payload).select().single();
    if (error) { showToast(error.message,"error"); return; }
    setPayments(p=>[data,...p]); showToast("Payment recorded","success"); setShowAddPayment(false); setPForm(pBlank);
  };

  const saveMaint = async () => {
    if (!mForm.tenant_id||!mForm.title.trim()) { showToast("Tenant and title required","error"); return; }
    const payload = { ...mForm, estimated_cost:Number(mForm.estimated_cost)||null, created_by:currentUser.id };
    const { data,error } = await supabase.from("maintenance").insert(payload).select().single();
    if (error) { showToast(error.message,"error"); return; }
    setMaintenance(p=>[data,...p]); showToast("Request created","success"); setShowAddMaint(false); setMForm(mBlank);
  };

  const updatePaymentStatus = async (id, status) => {
    await supabase.from("rent_payments").update({ status, payment_date: status==="Received"||status==="Cleared"?new Date().toISOString().slice(0,10):null }).eq("id",id);
    setPayments(p=>p.map(x=>x.id===id?{...x,status}:x)); showToast(`Payment ${status}","success`);
  };

  const updateMaintStatus = async (id, status) => {
    const upd = { status, completed_at: status==="Completed"?new Date().toISOString():null };
    await supabase.from("maintenance").update(upd).eq("id",id);
    setMaintenance(p=>p.map(m=>m.id===id?{...m,...upd}:m)); showToast("Status updated","success");
  };

  const tenantName = (id) => tenants.find(t=>t.id===id)?.full_name||"Unknown";
  const unitRef    = (id) => pbUnits.find(u=>u.id===id)?.unit_ref||"—";
  const projName   = (uid) => { const u=pbUnits.find(x=>x.id===uid); return u?pbProjects.find(p=>p.id===u.project_id)?.name||"—":"—"; };

  // Expiry checks
  const today = new Date();
  const expiringLeases  = leases.filter(l=>{ if(l.status!=="Active") return false; const d=new Date(l.end_date); const days=(d-today)/(86400000); return days>=0&&days<=90; });
  const overduePayments = payments.filter(p=>p.status==="Pending"&&new Date(p.due_date)<today);
  ...
  const stats = {
    ...
    totalRent:     leases.filter(l=>l.status==="Active").reduce((s,l)=>s+(l.annual_rent||0),0),
    pending:       payments.filter(p=>p.status==="Pending").length,
    overdue:       overduePayments.length,
    expiring90:    expiringLeases.length,
    openMaint:     openMaint.length,
  };

  const PRIORITY_COLOR = { Emergency:{c:"#B83232",bg:"#FAEAEA"}, High:{c:"#B85C10",bg:"#FDF0E6"}, Medium:{c:"#A06810",bg:"#FDF3DC"}, Low:{c:"#1A5FA8",bg:"#E6EFF9"} };
  const MAINT_STATUS_COLOR = { Open:{c:"#B83232",bg:"#FAEAEA"}, "In Progress":{c:"#A06810",bg:"#FDF3DC"}, Completed:{c:"#1A7F5A",bg:"#E6F4EE"}, "On Hold":{c:"#718096",bg:"#F0F2F5"}, Cancelled:{c:"#718096",bg:"#F0F2F5"} };
  const PAY_STATUS_COLOR   = { Pending:{c:"#A06810",bg:"#FDF3DC"}, Received:{c:"#1A5FA8",bg:"#E6EFF9"}, Cleared:{c:"#1A7F5A",bg:"#E6F4EE"}, Bounced:{c:"#B83232",bg:"#FAEAEA"}, Cancelled:{c:"#718096",bg:"#F0F2F5"}, Waived:{c:"#718096",bg:"#F0F2F5"} };
  ...
  return (
    <div className="fade-in" style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      <div style={{ display:"flex", gap:4, marginBottom:14, flexWrap:"wrap" }}>
        {TABS_L.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)}
            style={{ padding:"7px 14px", borderRadius:8, border:`1.5px solid ${tab===t.id?"#0B1F3A":"#E2E8F0"}`, background:tab===t.id?"#0B1F3A":"#fff", color:tab===t.id?"#fff":"#4A5568", fontSize:12, fontWeight:tab===t.id?600:400, cursor:"pointer", display:"flex", alignItems:"center", gap:5 }}>
            {t.icon} {t.label}
            {t.id==="renewals"&&stats.expiring90>0&&<span style={{ background:"#B83232", color:"#fff", borderRadius:20, fontSize:10, padding:"0 5px", fontWeight:700 }}>{stats.expiring90}</span>}
            {t.id==="payments"&&stats.overdue>0&&<span style={{ background:"#B83232", color:"#fff", borderRadius:20, fontSize:10, padding:"0 5px", fontWeight:700 }}>{stats.overdue}</span>}
          </button>
        ))}
      </div>
      ...
      {tab==="dashboard" && (
        <div style={{ flex:1, overflowY:"auto" }}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:16 }}>
            {[["Active Leases",stats.activeleases,"#0B1F3A","📄"],["Active Tenants",stats.activeTenants,"#1A5FA8","🧑"],["Annual Rent Portfolio",fmtFull(stats.totalRent),"#1A7F5A","💰"],["Pending Payments",stats.pending,"#A06810","💳"]].map(([l,v,c,icon])=>(
              <div key={l} style={{ background:"#fff", border:"1px solid #E2E8F0", borderRadius:12, padding:"1rem 1.25rem", borderTop:`3px solid ${c}` }}>
                ...
                <div style={{ fontSize:10, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.6px", marginBottom:4 }}>{l}</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12 }}>
            ...
            <div style={{ background:"#fff", border:"1px solid #E2E8F0", borderRadius:12, padding:"1rem" }}>
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:14, fontWeight:700, color:"#B83232", marginBottom:10 }}>⚠ Overdue Payments ({stats.overdue})</div>
              {overduePayments.length===0 && <div style={{ fontSize:12, color:"#A0AEC0" }}>All payments up to date ✓</div>}
              {overduePayments.slice(0,4).map(p=>(
                <div key={p.id} style={{ padding:"7px 0", borderBottom:"1px solid #F0F2F5", fontSize:12 }}>
                  <div style={{ fontWeight:600, color:"#0B1F3A" }}>{tenantName(p.tenant_id)}</div>
                  <div style={{ color:"#B83232" }}>AED {Number(p.amount).toLocaleString()} · Due {fmtDate(p.due_date)}</div>
                </div>
              ))}
            </div>
            {/* Expiring */}
            <div style={{ background:"#fff", border:"1px solid #E2E8F0", borderRadius:12, padding:"1rem" }}>
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:14, fontWeight:700, color:"#A06810", marginBottom:10 }}>🔄 Expiring within 90 days ({stats.expiring90})</div>
              {expiringLeases.length===0 && <div style={{ fontSize:12, color:"#A0AEC0" }}>No leases expiring soon ✓</div>}
              {expiringLeases.slice(0,4).map(l=>{
                const days=Math.ceil((new Date(l.end_date)-today)/86400000);
                return (
                  <div key={l.id} style={{ padding:"7px 0", borderBottom:"1px solid #F0F2F5", fontSize:12 }}>
                    <div style={{ fontWeight:600, color:"#0B1F3A" }}>{tenantName(l.tenant_id)}</div>
                    <div style={{ color:"#A06810" }}>{unitRef(l.unit_id)} · Expires {fmtDate(l.end_date)} ({days}d)</div>
                  </div>
                );
              })}
            </div>
            {/* Maintenance */}
            <div style={{ background:"#fff", border:"1px solid #E2E8F0", borderRadius:12, padding:"1rem" }}>
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:14, fontWeight:700, color:"#1A5FA8", marginBottom:10 }}>🔧 Open Maintenance ({stats.openMaint})</div>
              {openMaint.length===0 && <div style={{ fontSize:12, color:"#A0AEC0" }}>No open requests ✓</div>}
              {openMaint.slice(0,4).map(m=>{ const pc=PRIORITY_COLOR[m.priority]||{}; return (
                <div key={m.id} style={{ padding:"7px 0", borderBottom:"1px solid #F0F2F5", fontSize:12 }}>
                  <div style={{ display:"flex", justifyContent:"space-between" }}>
                    <span style={{ fontWeight:600, color:"#0B1F3A" }}>{m.title.slice(0,28)}</span>
                    <span style={{ fontSize:10, fontWeight:600, padding:"1px 6px", borderRadius:20, background:pc.bg, color:pc.c }}>{m.priority}</span>
                  </div>
                  <div style={{ color:"#A0AEC0" }}>{tenantName(m.tenant_id)} · {m.category}</div>
                </div>
              );})}
            </div>
          </div>
        ...
        <div style={{ display:"flex", gap:14, flex:1, overflow:"hidden" }}>
          <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
              ...
              {canEdit && <Btn onClick={()=>{ setTForm(tBlank); setShowAddTenant(true); }}>+ Add Tenant</Btn>}
            </div>
            <div style={{ flex:1, overflowY:"auto" }}>
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                <thead style={{ position:"sticky", top:0 }}>
                  <tr style={{ background:"#0B1F3A" }}>
                    {["Tenant","Type","Phone","Nationality","Passport Exp","Visa Exp","Status","Leases"].map(h=>(
                      <th key={h} style={{ padding:"9px 12px", textAlign:"left", fontSize:11, fontWeight:600, color:"#C9A84C", textTransform:"uppercase", letterSpacing:"0.5px", whiteSpace:"nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {tenants.map((t,i)=>{
                    const tleases = leases.filter(l=>l.tenant_id===t.id&&l.status==="Active").length;
                    const passExp = t.passport_expiry && new Date(t.passport_expiry)<today;
                    const visaExp = t.visa_expiry     && new Date(t.visa_expiry)<today;
                    return (
                      <tr key={t.id} style={{ background:i%2===0?"#fff":"#FAFBFC", borderBottom:"1px solid #F0F2F5", cursor:"pointer" }} onClick={()=>setSelTenant(selTenant?.id===t.id?null:t)}>
                        <td style={{ padding:"10px 12px" }}>
                          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                            <Av name={t.full_name} size={30}/>
                            <div>
                              <div style={{ fontSize:13, fontWeight:600, color:"#0B1F3A" }}>{t.full_name}</div>
                              <div style={{ fontSize:11, color:"#A0AEC0" }}>{t.email}</div>
                            </div>
                          </div>
                        </td>
                        <td style={{ padding:"10px 12px", fontSize:12, color:"#4A5568" }}>{t.tenant_type}{t.company_name?` — ${t.company_name}`:""}</td>
                        ...
                        <td style={{ padding:"10px 12px", fontSize:12, color:passExp?"#B83232":"#4A5568", fontWeight:passExp?600:400 }}>{t.passport_expiry?fmtDate(t.passport_expiry):"—"}{passExp?" ⚠":""}</td>
                        <td style={{ padding:"10px 12px", fontSize:12, color:visaExp?"#B83232":"#4A5568", fontWeight:visaExp?600:400 }}>{t.visa_expiry?fmtDate(t.visa_expiry):"—"}{visaExp?" ⚠":""}</td>
                        <td style={{ padding:"10px 12px" }}><span style={{ fontSize:11, fontWeight:600, padding:"2px 8px", borderRadius:20, background:t.status==="Active"?"#E6F4EE":t.status==="Blacklisted"?"#FAEAEA":"#F0F2F5", color:t.status==="Active"?"#1A7F5A":t.status==="Blacklisted"?"#B83232":"#718096" }}>{t.status}</span></td>
                        <td style={{ padding:"10px 12px", fontSize:13, fontWeight:700, color:"#0B1F3A" }}>{tleases}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          {selTenant && (
            <div className="slide-in" style={{ width:280, background:"#fff", border:"1px solid #E2E8F0", borderRadius:12, flexShrink:0, overflow:"hidden", display:"flex", flexDirection:"column" }}>
              <div style={{ background:"#0B1F3A", padding:"1.25rem", position:"relative" }}>
                <button onClick={()=>setSelTenant(null)} style={{ position:"absolute", top:10, right:12, background:"none", border:"none", color:"#C9A84C", fontSize:20, cursor:"pointer" }}>×</button>
                <Av name={selTenant.full_name} size={44} bg="#C9A84C" tc="#0B1F3A"/>
                <div style={{ fontFamily:"'Playfair Display',serif", fontSize:16, color:"#fff", fontWeight:700, marginTop:8 }}>{selTenant.full_name}</div>
                {selTenant.company_name && <div style={{ fontSize:12, color:"#C9A84C" }}>{selTenant.company_name}</div>}
              </div>
              <div style={{ flex:1, overflowY:"auto", padding:"1rem" }}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, background:"#FAFBFC", borderRadius:10, padding:"10px", marginBottom:12 }}>
                  <FR label="Type"        value={selTenant.tenant_type}/>
                  <FR label="Nationality" value={selTenant.nationality||"—"}/>
                  <FR label="Phone"       value={selTenant.phone||"—"}/>
                  <FR label="WhatsApp"    value={selTenant.whatsapp||"—"}/>
                  <FR label="Passport"    value={selTenant.passport_no||"—"}/>
                  <FR label="Ppt Expiry"  value={fmtDate(selTenant.passport_expiry)}/>
                  <FR label="Visa No"     value={selTenant.visa_no||"—"}/>
                  <FR label="Visa Expiry" value={fmtDate(selTenant.visa_expiry)}/>
                  <FR label="Emirates ID" value={selTenant.emirates_id||"—"}/>
                </div>
                <div style={{ marginBottom:10 }}>
                  <div style={{ fontSize:11, fontWeight:600, color:"#A0AEC0", marginBottom:6, textTransform:"uppercase" }}>Active Leases</div>
                  {leases.filter(l=>l.tenant_id===selTenant.id&&l.status==="Active").map(l=>(
                    <div key={l.id} style={{ padding:"7px 10px", background:"#E6EFF9", borderRadius:8, marginBottom:5 }}>
                      <div style={{ fontSize:13, fontWeight:600, color:"#0B1F3A" }}>{unitRef(l.unit_id)}</div>
                      <div style={{ fontSize:11, color:"#1A5FA8" }}>{fmtFull(l.annual_rent)}/yr · Expires {fmtDate(l.end_date)}</div>
                    </div>
                  ))}
                  {leases.filter(l=>l.tenant_id===selTenant.id&&l.status==="Active").length===0 && <div style={{ fontSize:12, color:"#A0AEC0" }}>No active leases</div>}
                </div>
              </div>
            </div>
          )}
        ...
        <div style={{ display:"flex", gap:14, flex:1, overflow:"hidden" }}>
          <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
              ...
              {canEdit && <Btn onClick={()=>{ setLForm(lBlank); setShowAddLease(true); }}>+ New Lease</Btn>}
            </div>
            <div style={{ flex:1, overflowY:"auto" }}>
              <table style={{ width:"100%", borderCollapse:"collapse" }}>
                <thead style={{ position:"sticky", top:0 }}>
                  <tr style={{ background:"#0B1F3A" }}>
                    {["Tenant","Unit","Project","Annual Rent","Start","End","Cheques","Ejari","Status"].map(h=>(
                      <th key={h} style={{ padding:"9px 12px", textAlign:"left", fontSize:11, fontWeight:600, color:"#C9A84C", textTransform:"uppercase", letterSpacing:"0.5px", whiteSpace:"nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {leases.map((l,i)=>{
                    const daysLeft = Math.ceil((new Date(l.end_date)-today)/86400000);
                    const expiring = daysLeft>=0&&daysLeft<=90&&l.status==="Active";
                    const expired  = daysLeft<0&&l.status==="Active";
                    return (
                      <tr key={l.id} style={{ background:i%2===0?"#fff":"#FAFBFC", borderBottom:"1px solid #F0F2F5", cursor:"pointer" }} onClick={()=>setSelLease(selLease?.id===l.id?null:l)}>
                        <td style={{ padding:"10px 12px", fontSize:13, fontWeight:600, color:"#0B1F3A" }}>{tenantName(l.tenant_id)}</td>
                        <td style={{ padding:"10px 12px", fontSize:12, fontWeight:600, color:"#1A5FA8" }}>{unitRef(l.unit_id)}</td>
                        <td style={{ padding:"10px 12px", fontSize:12, color:"#4A5568" }}>{projName(l.unit_id)}</td>
                        <td style={{ padding:"10px 12px", fontSize:13, fontWeight:700, color:"#0B1F3A" }}>{fmtFull(l.annual_rent)}</td>
                        <td style={{ padding:"10px 12px", fontSize:12, color:"#4A5568" }}>{fmtDate(l.start_date)}</td>
                        <td style={{ padding:"10px 12px", fontSize:12, color:expired?"#B83232":expiring?"#A06810":"#4A5568", fontWeight:expired||expiring?600:400 }}>{fmtDate(l.end_date)}{expiring?` (${daysLeft}d)`:""}{expired?" Expired!":""}</td>
                        <td style={{ padding:"10px 12px", fontSize:12, color:"#4A5568" }}>{l.no_of_cheques||4}</td>
                        <td style={{ padding:"10px 12px", fontSize:12, color:l.ejari_no?"#1A7F5A":"#A0AEC0" }}>{l.ejari_no||"Pending"}</td>
                        <td style={{ padding:"10px 12px" }}><span style={{ fontSize:11, fontWeight:600, padding:"2px 8px", borderRadius:20, background:l.status==="Active"?"#E6F4EE":l.status==="Expired"?"#FAEAEA":l.status==="Under Renewal"?"#FDF3DC":"#F0F2F5", color:l.status==="Active"?"#1A7F5A":l.status==="Expired"?"#B83232":l.status==="Under Renewal"?"#A06810":"#718096" }}>{l.status}</span></td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          {/* Lease detail */}
          {selLease && (
            <div className="slide-in" style={{ width:270, background:"#fff", border:"1px solid #E2E8F0", borderRadius:12, flexShrink:0, overflow:"hidden", display:"flex", flexDirection:"column" }}>
              <div style={{ background:"#0B1F3A", padding:"1.25rem", position:"relative" }}>
                <button onClick={()=>setSelLease(null)} style={{ position:"absolute", top:10, right:12, background:"none", border:"none", color:"#C9A84C", fontSize:20, cursor:"pointer" }}>×</button>
                <div style={{ fontFamily:"'Playfair Display',serif", fontSize:16, color:"#C9A84C", fontWeight:700 }}>{unitRef(selLease.unit_id)}</div>
                <div style={{ fontSize:13, color:"#fff", marginTop:4 }}>{tenantName(selLease.tenant_id)}</div>
                <div style={{ fontSize:11, color:"rgba(255,255,255,0.6)", marginTop:2 }}>{projName(selLease.unit_id)}</div>
              </div>
              <div style={{ flex:1, overflowY:"auto", padding:"1rem" }}>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, background:"#FAFBFC", borderRadius:10, padding:"10px", marginBottom:12 }}>
                  <FR label="Annual Rent"   value={fmtFull(selLease.annual_rent)}/>
                  <FR label="Cheques"       value={selLease.no_of_cheques||4}/>
                  <FR label="Start"         value={fmtDate(selLease.start_date)}/>
                  <FR label="End"           value={fmtDate(selLease.end_date)}/>
                  <FR label="Security Dep." value={selLease.security_deposit?fmtFull(selLease.security_deposit):"—"}/>
                  <FR label="Municipality"  value={selLease.municipality_fee?fmtFull(selLease.municipality_fee):"—"}/>
                  <FR label="Ejari No."     value={selLease.ejari_no||"—"}/>
                  <FR label="Contract No."  value={selLease.contract_no||"—"}/>
                </div>
                {/* Payments for this lease */}
                <div style={{ fontSize:11, fontWeight:600, color:"#A0AEC0", marginBottom:8, textTransform:"uppercase" }}>Payments</div>
                {payments.filter(p=>p.lease_id===selLease.id).slice(0,5).map(p=>{
                  const ps=PAY_STATUS_COLOR[p.status]||{};
                  return (
                    <div key={p.id} style={{ display:"flex", justifyContent:"space-between", padding:"6px 0", borderBottom:"1px solid #F0F2F5", fontSize:12 }}>
                      <div>
                        <span style={{ fontWeight:600, color:"#0B1F3A" }}>AED {Number(p.amount).toLocaleString()}</span>
                        <span style={{ color:"#A0AEC0", marginLeft:6 }}>Due {fmtDate(p.due_date)}</span>
                      </div>
                      <span style={{ fontSize:10, fontWeight:600, padding:"1px 7px", borderRadius:20, background:ps.bg, color:ps.c }}>{p.status}</span>
                    </div>
                  );
                })}
                <div style={{ display:"flex", flexDirection:"column", gap:7, marginTop:12 }}>
                  {canEdit && <Btn full variant="gold" onClick={()=>{ setPForm({...pBlank,lease_id:selLease.id}); setShowAddPayment(true); }}>+ Record Payment</Btn>}
                  {canEdit && <Btn full variant="outline" onClick={async()=>{ await supabase.from("leases").update({status:"Under Renewal"}).eq("id",selLease.id); setLeases(p=>p.map(l=>l.id===selLease.id?{...l,status:"Under Renewal"}:l)); showToast("Marked for renewal","success"); }}>🔄 Start Renewal</Btn>}
                </div>
              </div>
            </div>
          )}
        ...
        <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
            <div style={{ display:"flex", gap:10 }}>
              <div style={{ fontSize:12, background:"#FAEAEA", color:"#B83232", padding:"4px 12px", borderRadius:20, fontWeight:600 }}>⚠ {overduePayments.length} overdue</div>
              <div style={{ fontSize:12, background:"#FDF3DC", color:"#A06810", padding:"4px 12px", borderRadius:20, fontWeight:600 }}>⏳ {payments.filter(p=>p.status==="Pending").length} pending</div>
            </div>
            {canEdit && <Btn onClick={()=>{ setPForm(pBlank); setShowAddPayment(true); }}>+ Record Payment</Btn>}
          </div>
          <div style={{ flex:1, overflowY:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              <thead style={{ position:"sticky", top:0 }}>
                <tr style={{ background:"#0B1F3A" }}>
                  {["Tenant","Unit","Amount","Due Date","Cheque No","Bank","Method","Status","Actions"].map(h=>(
                    <th key={h} style={{ padding:"9px 12px", textAlign:"left", fontSize:11, fontWeight:600, color:"#C9A84C", textTransform:"uppercase", letterSpacing:"0.5px", whiteSpace:"nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
                ...
                {payments.map((p,i)=>{
                  ...
                  return (
                    <tr key={p.id} style={{ background:isOverdue?"#FFF5F5":i%2===0?"#fff":"#FAFBFC", borderBottom:"1px solid #F0F2F5" }}>
                      <td style={{ padding:"9px 12px", fontSize:13, fontWeight:600, color:"#0B1F3A" }}>{tenantName(p.tenant_id)}</td>
                      <td style={{ padding:"9px 12px", fontSize:12, color:"#1A5FA8", fontWeight:600 }}>{unitRef(p.unit_id)}</td>
                      <td style={{ padding:"9px 12px", fontSize:13, fontWeight:700, color:"#0B1F3A" }}>AED {Number(p.amount).toLocaleString()}</td>
                      <td style={{ padding:"9px 12px", fontSize:12, color:isOverdue?"#B83232":"#4A5568", fontWeight:isOverdue?600:400 }}>{fmtDate(p.due_date)}{isOverdue?" ⚠":""}</td>
                      <td style={{ padding:"9px 12px", fontSize:12, color:"#4A5568" }}>{p.cheque_no||"—"}</td>
                      ...
                      <td style={{ padding:"9px 12px" }}><span style={{ fontSize:10, fontWeight:600, padding:"2px 8px", borderRadius:20, background:ps.bg, color:ps.c }}>{p.status}</span></td>
                      <td style={{ padding:"9px 12px" }}>
                        {p.status==="Pending"&&canEdit && (
                          <div style={{ display:"flex", gap:4 }}>
                            <button onClick={()=>updatePaymentStatus(p.id,"Received")} style={{ fontSize:10, padding:"3px 8px", borderRadius:6, border:"none", background:"#E6F4EE", color:"#1A7F5A", cursor:"pointer", fontWeight:600 }}>Received</button>
                            <button onClick={()=>updatePaymentStatus(p.id,"Bounced")} style={{ fontSize:10, padding:"3px 8px", borderRadius:6, border:"none", background:"#FAEAEA", color:"#B83232", cursor:"pointer", fontWeight:600 }}>Bounced</button>
                          </div>
                        )}
                        {p.status==="Received"&&canEdit && (
                          <button onClick={()=>updatePaymentStatus(p.id,"Cleared")} style={{ fontSize:10, padding:"3px 8px", borderRadius:6, border:"none", background:"#E6F4EE", color:"#1A7F5A", cursor:"pointer", fontWeight:600 }}>Mark Cleared</button>
                        )}
                      </td>
                    </tr>
                  );
            ...
            {payments.length===0 && <Empty icon="💳" msg="No payments recorded yet"/>}
          </div>
        </div>
      )}
      ...
      {tab==="renewals" && (
        <div style={{ flex:1, overflowY:"auto" }}>
          {expiringLeases.length===0 && <Empty icon="🔄" msg="No leases expiring within 90 days — all good!"/>}
          {expiringLeases.length>0 && (
            <>
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:15, fontWeight:700, color:"#0B1F3A", marginBottom:14 }}>Leases Requiring Renewal Attention</div>
              {[["≤14 days",14,"#B83232"],["≤30 days",30,"#B85C10"],["≤60 days",60,"#A06810"],["≤90 days",90,"#1A5FA8"]].map(([label,days,color])=>{
                const group = expiringLeases.filter(l=>{ const d=Math.ceil((new Date(l.end_date)-today)/86400000); return d>=0&&d<=(days===14?14:days===30?30:days===60?60:90)&&(days===14||d>14)&&(days===30||days===14||d>30)&&(days===60||days===30||days===14||d>60); });
                if (days!==14&&days!==30&&days!==60&&days!==90) return null;
                const g = expiringLeases.filter(l=>{ const d=Math.ceil((new Date(l.end_date)-today)/86400000); if(days===14) return d<=14; if(days===30) return d>14&&d<=30; if(days===60) return d>30&&d<=60; return d>60&&d<=90; });
                if (g.length===0) return null;
                return (
                  <div key={label} style={{ marginBottom:18 }}>
                    <div style={{ fontSize:12, fontWeight:700, color, textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:8 }}>Expiring in {label} ({g.length})</div>
                    {g.map(l=>{
                      const daysLeft=Math.ceil((new Date(l.end_date)-today)/86400000);
                      return (
                        <div key={l.id} style={{ display:"flex", gap:12, padding:"12px 14px", background:"#fff", border:`1.5px solid ${daysLeft<=14?"#F0BCBC":daysLeft<=30?"#F5D5A8":"#E2E8F0"}`, borderRadius:10, marginBottom:8, alignItems:"center" }}>
                          <Av name={tenantName(l.tenant_id)} size={36}/>
                          <div style={{ flex:1 }}>
                            <div style={{ fontSize:13, fontWeight:700, color:"#0B1F3A" }}>{tenantName(l.tenant_id)}</div>
                            <div style={{ fontSize:12, color:"#4A5568" }}>{unitRef(l.unit_id)} · {fmtFull(l.annual_rent)}/yr</div>
                            <div style={{ fontSize:11, color }}>Expires {fmtDate(l.end_date)} · {daysLeft} days left</div>
                          </div>
                          <div style={{ display:"flex", gap:6 }}>
                            {canEdit && <Btn small variant="gold" onClick={async()=>{ await supabase.from("leases").update({status:"Under Renewal"}).eq("id",l.id); setLeases(p=>p.map(x=>x.id===l.id?{...x,status:"Under Renewal"}:x)); showToast("Marked for renewal","success"); }}>Start Renewal</Btn>}
                            <Btn small variant="outline" onClick={()=>setSelLease(l)}>View</Btn>
                          </div>
                        </div>
                      );
                    })}
      ...
      {tab==="maint" && (
        <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
            <span style={{ fontSize:12, color:"#A0AEC0" }}>{maintenance.length} request{maintenance.length!==1?"s":""} · {openMaint.length} open</span>
            {canEdit && <Btn onClick={()=>{ setMForm(mBlank); setShowAddMaint(true); }}>+ New Request</Btn>}
          </div>
          <div style={{ flex:1, overflowY:"auto" }}>
            {maintenance.map(m=>{
              const pc=PRIORITY_COLOR[m.priority]||{}; const mc=MAINT_STATUS_COLOR[m.status]||{};
              return (
                <div key={m.id} style={{ display:"flex", gap:12, padding:"12px 14px", background:"#fff", border:"1px solid #E2E8F0", borderRadius:10, marginBottom:8 }}>
                  <div style={{ flex:1 }}>
                    <div style={{ display:"flex", gap:8, alignItems:"center", marginBottom:5, flexWrap:"wrap" }}>
                      <span style={{ fontSize:13, fontWeight:700, color:"#0B1F3A" }}>{m.title}</span>
                      <span style={{ fontSize:10, fontWeight:600, padding:"2px 7px", borderRadius:20, background:pc.bg, color:pc.c }}>{m.priority}</span>
                      <span style={{ fontSize:10, fontWeight:600, padding:"2px 7px", borderRadius:20, background:mc.bg, color:mc.c }}>{m.status}</span>
                    </div>
                    <div style={{ fontSize:12, color:"#4A5568", marginBottom:4 }}>{tenantName(m.tenant_id)} · {unitRef(m.unit_id)} · {m.category}</div>
                    {m.description && <div style={{ fontSize:12, color:"#718096", lineHeight:1.5 }}>{m.description}</div>}
                    {m.contractor_name && <div style={{ fontSize:11, color:"#A0AEC0", marginTop:4 }}>Contractor: {m.contractor_name}{m.contractor_phone?` · ${m.contractor_phone}`:""}</div>}
                    <div style={{ display:"flex", gap:8, marginTop:4 }}>
                      {m.estimated_cost && <span style={{ fontSize:11, color:"#A0AEC0" }}>Est: AED {Number(m.estimated_cost).toLocaleString()}</span>}
                      {m.actual_cost    && <span style={{ fontSize:11, color:"#0B1F3A", fontWeight:600 }}>Actual: AED {Number(m.actual_cost).toLocaleString()}</span>}
                      <span style={{ fontSize:11, color:"#A0AEC0" }}>Charged to: {m.charged_to||"TBD"}</span>
                    </div>
                  </div>
                  {canEdit && m.status!=="Completed"&&m.status!=="Cancelled" && (
                    <div style={{ display:"flex", flexDirection:"column", gap:5, flexShrink:0 }}>
                      {m.status==="Open" && <Btn small variant="outline" onClick={()=>updateMaintStatus(m.id,"In Progress")}>Start</Btn>}
                      {m.status==="In Progress" && <Btn small variant="green" onClick={()=>updateMaintStatus(m.id,"Completed")}>Complete</Btn>}
                      <Btn small variant="danger" onClick={()=>updateMaintStatus(m.id,"Cancelled")}>Cancel</Btn>
                    </div>
                  )}
                </div>
              );
            })}
            {maintenance.length===0 && <Empty icon="🔧" msg="No maintenance requests — all good!"/>}
          </div>
        </div>
      )}
      ...
      {/* ── MODALS ────────────────────────────────────── */}
      {showAddTenant && (
        <Modal title="Add New Tenant" onClose={()=>setShowAddTenant(false)} width={580}>
          <G2><FF label="Full Name" required><input value={tForm.full_name} onChange={e=>setTForm(f=>({...f,full_name:e.target.value}))} placeholder="Ahmed Al Mansoori"/></FF>
          <FF label="Tenant Type"><select value={tForm.tenant_type} onChange={e=>setTForm(f=>({...f,tenant_type:e.target.value}))}><option>Individual</option><option>Company</option></select></FF></G2>
          {tForm.tenant_type==="Company" && <G2><FF label="Company Name"><input value={tForm.company_name} onChange={e=>setTForm(f=>({...f,company_name:e.target.value}))}/></FF>
          <FF label="Trade License No."><input value={tForm.trade_license_no} onChange={e=>setTForm(f=>({...f,trade_license_no:e.target.value}))}/></FF></G2>}
          <G2><FF label="Phone"><input value={tForm.phone} onChange={e=>setTForm(f=>({...f,phone:e.target.value}))} placeholder="+971 50 000 0000"/></FF>
          <FF label="WhatsApp"><input value={tForm.whatsapp} onChange={e=>setTForm(f=>({...f,whatsapp:e.target.value}))} placeholder="+971 50 000 0000"/></FF></G2>
          <G2><FF label="Email"><input type="email" value={tForm.email} onChange={e=>setTForm(f=>({...f,email:e.target.value}))}/></FF>
          <FF label="Nationality"><select value={tForm.nationality} onChange={e=>setTForm(f=>({...f,nationality:e.target.value}))}><option value="">Select…</option>{NATIONALITIES.map(n=><option key={n}>{n}</option>)}</select></FF></G2>
          <div style={{ fontSize:11, fontWeight:700, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.5px", margin:"12px 0 10px" }}>Documents</div>
          <G2><FF label="Passport No."><input value={tForm.passport_no} onChange={e=>setTForm(f=>({...f,passport_no:e.target.value}))}/></FF>
          <FF label="Passport Expiry"><input type="date" value={tForm.passport_expiry} onChange={e=>setTForm(f=>({...f,passport_expiry:e.target.value}))}/></FF></G2>
          <G2><FF label="Visa No."><input value={tForm.visa_no} onChange={e=>setTForm(f=>({...f,visa_no:e.target.value}))}/></FF>
          <FF label="Visa Expiry"><input type="date" value={tForm.visa_expiry} onChange={e=>setTForm(f=>({...f,visa_expiry:e.target.value}))}/></FF></G2>
          <G2><FF label="Emirates ID"><input value={tForm.emirates_id} onChange={e=>setTForm(f=>({...f,emirates_id:e.target.value}))}/></FF>
          <FF label="Emirates ID Expiry"><input type="date" value={tForm.emirates_id_expiry} onChange={e=>setTForm(f=>({...f,emirates_id_expiry:e.target.value}))}/></FF></G2>
          <FF label="Notes"><textarea value={tForm.notes} onChange={e=>setTForm(f=>({...f,notes:e.target.value}))} rows={2}/></FF>
          <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}><Btn variant="outline" onClick={()=>setShowAddTenant(false)}>Cancel</Btn><Btn onClick={saveTenant}>Save Tenant</Btn></div>
        </Modal>
      )}
      {showAddLease && (
        <Modal title="New Lease Contract" onClose={()=>setShowAddLease(false)} width={560}>
          <G2><FF label="Tenant" required><select value={lForm.tenant_id} onChange={e=>setLForm(f=>({...f,tenant_id:e.target.value}))}><option value="">Select tenant…</option>{tenants.filter(t=>t.status==="Active").map(t=><option key={t.id} value={t.id}>{t.full_name}</option>)}</select></FF>
          <FF label="Unit" required><select value={lForm.unit_id} onChange={e=>setLForm(f=>({...f,unit_id:e.target.value}))}><option value="">Select unit…</option>{pbUnits.map(u=><option key={u.id} value={u.id}>{u.unit_ref}{pbProjects.find(p=>p.id===u.project_id)?` — ${pbProjects.find(p=>p.id===u.project_id).name}`:""}</option>)}</select></FF></G2>
          <G2><FF label="Lease Type"><select value={lForm.lease_type} onChange={e=>setLForm(f=>({...f,lease_type:e.target.value}))}><option>Residential</option><option>Commercial</option></select></FF>
          <FF label="Contract No."><input value={lForm.contract_no} onChange={e=>setLForm(f=>({...f,contract_no:e.target.value}))} placeholder="e.g. RERA-2024-00123"/></FF></G2>
          ...
          <G2><FF label="Annual Rent (AED)" required><input type="number" value={lForm.annual_rent} onChange={e=>setLForm(f=>({...f,annual_rent:e.target.value}))} placeholder="120000"/></FF>
          <FF label="Security Deposit (AED)"><input type="number" value={lForm.security_deposit} onChange={e=>setLForm(f=>({...f,security_deposit:e.target.value}))}/></FF></G2>
          <G2><FF label="No. of Cheques"><select value={lForm.no_of_cheques} onChange={e=>setLForm(f=>({...f,no_of_cheques:e.target.value}))}>{CHEQUE_OPTS.map(n=><option key={n}>{n}</option>)}</select></FF>
          <FF label="Agency Commission (AED)"><input type="number" value={lForm.agency_commission} onChange={e=>setLForm(f=>({...f,agency_commission:e.target.value}))}/></FF></G2>
          <G2><FF label="Ejari No."><input value={lForm.ejari_no} onChange={e=>setLForm(f=>({...f,ejari_no:e.target.value}))}/></FF>
          <FF label="RERA No."><input value={lForm.rera_no} onChange={e=>setLForm(f=>({...f,rera_no:e.target.value}))}/></FF></G2>
          <FF label="Notes"><textarea value={lForm.notes} onChange={e=>setLForm(f=>({...f,notes:e.target.value}))} rows={2}/></FF>
          <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}><Btn variant="outline" onClick={()=>setShowAddLease(false)}>Cancel</Btn><Btn onClick={saveLease}>Create Lease</Btn></div>
        </Modal>
      )}
      {showAddPayment && (
        <Modal title="Record Payment" onClose={()=>setShowAddPayment(false)} width={480}>
          <FF label="Lease" required><select value={pForm.lease_id} onChange={e=>setPForm(f=>({...f,lease_id:e.target.value}))}><option value="">Select lease…</option>{leases.filter(l=>l.status==="Active").map(l=><option key={l.id} value={l.id}>{tenantName(l.tenant_id)} — {unitRef(l.unit_id)}</option>)}</select></FF>
          <G2><FF label="Amount (AED)" required><input type="number" value={pForm.amount} onChange={e=>setPForm(f=>({...f,amount:e.target.value}))}/></FF>
          <FF label="Due Date" required><input type="date" value={pForm.due_date} onChange={e=>setPForm(f=>({...f,due_date:e.target.value}))}/></FF></G2>
          <G2><FF label="Payment Method"><select value={pForm.payment_method} onChange={e=>setPForm(f=>({...f,payment_method:e.target.value}))}>{["Cheque","Bank Transfer","Cash","Online","PDC"].map(m=><option key={m}>{m}</option>)}</select></FF>
          <FF label="Status"><select value={pForm.status} onChange={e=>setPForm(f=>({...f,status:e.target.value}))}>{["Pending","Received","Cleared","Bounced"].map(s=><option key={s}>{s}</option>)}</select></FF></G2>
          <G2><FF label="Cheque No."><input value={pForm.cheque_no} onChange={e=>setPForm(f=>({...f,cheque_no:e.target.value}))}/></FF>
          <FF label="Bank Name"><input value={pForm.bank_name} onChange={e=>setPForm(f=>({...f,bank_name:e.target.value}))}/></FF></G2>
          <G2><FF label="Period From"><input type="date" value={pForm.period_from} onChange={e=>setPForm(f=>({...f,period_from:e.target.value}))}/></FF>
          <FF label="Period To"><input type="date" value={pForm.period_to} onChange={e=>setPForm(f=>({...f,period_to:e.target.value}))}/></FF></G2>
          <FF label="Notes"><textarea value={pForm.notes} onChange={e=>setPForm(f=>({...f,notes:e.target.value}))} rows={2}/></FF>
          <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}><Btn variant="outline" onClick={()=>setShowAddPayment(false)}>Cancel</Btn><Btn variant="gold" onClick={savePayment}>Record Payment</Btn></div>
        </Modal>
      )}
      {showAddMaint && (
        <Modal title="New Maintenance Request" onClose={()=>setShowAddMaint(false)} width={520}>
          <G2><FF label="Tenant" required><select value={mForm.tenant_id} onChange={e=>setMForm(f=>({...f,tenant_id:e.target.value}))}><option value="">Select tenant…</option>{tenants.map(t=><option key={t.id} value={t.id}>{t.full_name}</option>)}</select></FF>
          <FF label="Unit"><select value={mForm.unit_id} onChange={e=>setMForm(f=>({...f,unit_id:e.target.value}))}><option value="">Select unit…</option>{pbUnits.map(u=><option key={u.id} value={u.id}>{u.unit_ref}</option>)}</select></FF></G2>
          <FF label="Title" required><input value={mForm.title} onChange={e=>setMForm(f=>({...f,title:e.target.value}))} placeholder="e.g. AC not working in master bedroom"/></FF>
          <G2><FF label="Category"><select value={mForm.category} onChange={e=>setMForm(f=>({...f,category:e.target.value}))}>{["Plumbing","Electrical","AC/HVAC","Structural","Painting","Cleaning","Security","Pest Control","Other"].map(c=><option key={c}>{c}</option>)}</select></FF>
          <FF label="Priority"><select value={mForm.priority} onChange={e=>setMForm(f=>({...f,priority:e.target.value}))}>{["Low","Medium","High","Emergency"].map(p=><option key={p}>{p}</option>)}</select></FF></G2>
          <FF label="Description"><textarea value={mForm.description} onChange={e=>setMForm(f=>({...f,description:e.target.value}))} rows={3}/></FF>
          <G2><FF label="Contractor Name"><input value={mForm.contractor_name} onChange={e=>setMForm(f=>({...f,contractor_name:e.target.value}))}/></FF>
          <FF label="Contractor Phone"><input value={mForm.contractor_phone} onChange={e=>setMForm(f=>({...f,contractor_phone:e.target.value}))}/></FF></G2>
          <G2><FF label="Estimated Cost (AED)"><input type="number" value={mForm.estimated_cost} onChange={e=>setMForm(f=>({...f,estimated_cost:e.target.value}))}/></FF>
          <FF label="Charged To"><select value={mForm.charged_to} onChange={e=>setMForm(f=>({...f,charged_to:e.target.value}))}>{["Owner","Tenant","Insurance","TBD"].map(c=><option key={c}>{c}</option>)}</select></FF></G2>
          <div style={{ display:"flex", gap:10, justifyContent:"flex-end" }}><Btn variant="outline" onClick={()=>setShowAddMaint(false)}>Cancel</Btn><Btn onClick={saveMaint}>Submit Request</Btn></div>
        </Modal>
      )}
    </div>
  );
...
// ═══════════════════════════════════════════════════════
// LEASING MODULE
// Tenants · Contracts · Payments · Renewals · Maintenance
// ═══════════════════════════════════════════════════════
