...
function OpportunityDetail({ opp, lead, opps, units, projects, salePricing, users, currentUser, showToast, onBack, onUpdated, onActivityLog }) {
  ...
  // Day 86: MONEY FOR A BLOCK CHILD IS COLLECTED AT THE BLOCK. Founder's sealed ruling: "payments
  // block, SPA line-wise - no two things in that decision." Fifteen units means fifteen entries
  // otherwise, and the allocator exists precisely to avoid that. If a broker recorded money here
  // instead, the block's Money tab would not know and the two views would diverge.
  // The table stays VISIBLE - he must still see what is owed and what the block has paid. Only
            ...
            <div style={{padding:"1.25rem 1.5rem",display:"flex",flexDirection:"column",gap:14}}>
              ...
              {(showStageGate==="SPA Signed"||showStageGate==="SPA Requirements")&&(<>
                ...
                {spaMode === "detailed" && (/* Pre-SPA gated */<div style={{padding:"12px 14px",background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:8}}>
                  ...
                  {moneyLocked && (
                    <div style={{margin:"0 0 10px",padding:"9px 12px",background:"#EEF2FF",border:"1.5px solid #C7D2FE",borderRadius:8,fontSize:11,fontWeight:600,color:"#3730A3"}}>
              ...
              <div style={{display:"flex",gap:10,justifyContent:"flex-end",paddingTop:8,borderTop:"1px solid #F1F5F9"}}>
                ...
                <button onClick={(stageGateViewMode && (opp.stage === "Closed Won" || opp.stage === "Closed Lost")) ? () => showToast("🔒 This deal is closed - records are view-only", "info") : stageGateViewMode ? () => setStageGateViewMode(false) : async()=>{
                  ...
                  // Day 91: the date guard was added TWICE in one sitting, so the broker was asked to
                  // confirm the same date twice - part of the "crazy" flow he described. One guard.
