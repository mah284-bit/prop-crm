      ...
      <div style={{ background: "#fff", borderRadius: 16, width: 720, maxWidth: "96vw", maxHeight: "92vh", overflow: "auto" }}>
        ...
        <div style={{ padding: "1.1rem 1.4rem" }}>
          <div style={{ background: after <= 0 && landed > 0 ? "#E6F4EE" : "#FFFBEB", border: "1px solid " + (after <= 0 && landed > 0 ? "#A7D8C3" : "#FCD34D"), borderRadius: 10, padding: "11px 14px", marginBottom: 14, display: "flex", gap: 26 }}>
            ...
            <div><div style={L}>{landed > 0 ? "Outstanding after this" : "Outstanding"}</div><div style={{ fontSize: 15, fontWeight: 800, color: (landed > 0 ? after : outstanding) <= 0 ? "#16A34A" : "#B91C1C" }}>{(landed > 0 ? after : outstanding) <= 0 ? "Nil" : fmt(landed > 0 ? after : outstanding)}</div></div>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
            ...
            <button onClick={submit} disabled={saving || landed <= 0} style={{ padding: "9px 20px", borderRadius: 8, border: "none", background: "#16A34A", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>{saving ? "Recording..." : "Record payment"}</button>
          </div>
        </div>
      </div>
    </div>
