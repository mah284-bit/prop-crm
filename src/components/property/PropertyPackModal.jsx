...
import { PROPERTY_PACK_EVENT, getPropertyPackContext } from "./propertyPackBus";
import PropertyPackShareModal from "./PropertyPackShareModal";
...
export default function PropertyPackModal() {
  ...
  return (
    <>
            ...
            {!loading && !error && pack && (
              <>
                ...
                <button
                  ...
                  title="Share Property Pack"
                  style={{ alignSelf: "flex-start", padding: "8px 16px", borderRadius: 8, border: "none", background: "#0F2540", color: "#fff", fontSize: 12, fontWeight: 600, cursor: "pointer" }}
                >
                  📤 Share Pack
                </button>
              </>
      ...
      <PropertyPackShareModal
        isOpen={showShare}
        onClose={() => setShowShare(false)}
        supabase={getPropertyPackContext().supabase}
        unit={pack?.unit}
        project={pack?.project}
        opportunity={getPropertyPackContext().opportunity}
        lead={getPropertyPackContext().lead}
        currentUser={getPropertyPackContext().currentUser}
        showToast={getPropertyPackContext().showToast}
      />
    </>
  );
}
