    ...
    <div className="fade-in" style={{display:"flex",flexDirection:"column",height:"100%"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",gap:6}}>
          ...
          {canEdit&&<button onClick={()=>{console.log("[EDIT] selLead:", selLead); setEditLeadForV2(selLead);setEditFormVersion(v=>v+1);setShowAddV2(true);}} style={{padding:"6px 14px",borderRadius:8,border:"1.5px solid #D1D9E6",background:"#fff",fontSize:12,fontWeight:600,cursor:"pointer"}}>✏ Edit</button>}
      ...
      <QuickProposalsPanel leadId={selLead.id} leadHasOpp={leadOpps.length > 0} onOpenOpp={() => { const o = leadOpps[0]; if (o && onNavigateToOpp) onNavigateToOpp(o); }} leadEmail={selLead.email} leadName={selLead.name} leadPhone={selLead.phone} company={currentUser.company || {}} currentUser={currentUser} onConvertUnit={(unitData) => { setPrefilledUnit(unitData); setShowCanonicalOppDialog(true); }} onPromote={handlePromoteProposal} />
        </div>
      </div>
