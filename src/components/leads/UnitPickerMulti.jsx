  ...
  return (
    <div ref={containerRef} style={{ background: "#fff", border: "1.5px solid #D1D9E6", borderRadius: 8, boxShadow: "0 14px 32px rgba(15,37,64,.15)", zIndex: 50, maxHeight: "80vh", display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 400 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {visible.length === 0 ? <div style={{ padding: "22px 12px", textAlign: "center", color: "#94A3B8" }}>No units match</div> : visible.map((u) => { const proj = (projects || []).find((p) => p.id === u.project_id); const bedLabel = u.bedrooms === 0 ? "Studio" : u.bedrooms ? `${u.bedrooms}BR` : ""; const isReserved = u.status === "Reserved" || u.status === "Sold"; const price = pricingMap[u.id]; const isSelected = selectedUnitIds.includes(u.id); return <div key={u.id} style={{ padding: "8px 12px", borderBottom: "1px solid #F1F5F9", display: "flex", gap: 10, alignItems: "center", opacity: isReserved ? 0.6 : 1, background: isSelected ? "#EFF6FF" : "#fff", cursor: "pointer" }}><input type="checkbox" checked={isSelected} onChange={() => toggleUnit(u.id)} disabled={isReserved} style={{ cursor: isReserved ? "not-allowed" : "pointer" }} /><div style={{ flex: 1 }}><div style={{ fontSize: 12, fontWeight: 700, color: "#0F2540" }}>{u.unit_ref} {isReserved && <span style={{ fontSize: 9, marginLeft: 6, padding: "1px 5px", borderRadius: 7, background: "#FEE2E2", color: "#C53030" }}>{u.status}</span>}</div><div style={{ fontSize: 10, color: "#64748B", marginTop: 1 }}>{[bedLabel, u.sub_type, proj?.name, u.size_sqft && `${u.size_sqft} sqft`].filter(Boolean).join(" · ")}</div></div>{price && <div style={{ fontSize: 11, fontWeight: 700, color: "#1A5FA8" }}>{fmtAed(price)}</div>}</div>; })}
      </div>
