...
const TABS=[
  ...
  {id:"permsets",   label:"Permissions",  icon:"🔐", app:"sales",   roles:["admin"]},
  // ── Leasing CRM ────────────────────────────────────────────────
  {id:"l_dashboard",label:"Dashboard",    icon:"⊞",  app:"leasing", roles:["admin","leasing_manager","leasing_agent","viewer"]},
  {id:"l_leads",    label:"Enquiries",    icon:"👤", app:"leasing", roles:["admin","leasing_manager","leasing_agent"]},
  ...
  {id:"l_permsets", label:"Permissions",  icon:"🔐", app:"leasing", roles:["admin"]},
];
...
function CompaniesModule({ currentUser, showToast, onSwitchCompany, activeCompanyId }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await db.from("companies").select("*, profiles(count)").order("name");
      setCompanies(data || []);
    } catch(e) {
