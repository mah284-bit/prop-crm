...
const MODE_TABS = {
  sales:   ["dashboard","leads","builder","pipeline","discounts","activity","ai","users"],
  leasing: ["dashboard","leasing","discounts","activity","ai","users"],
  both:    ["dashboard","leads","builder","pipeline","leasing","discounts","activity","ai","users"],
};
        ...
        <div className="fade-in" style={{flex:1,overflowY:"auto"}}>
          <div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:14,padding:"1.5rem",marginBottom:16}}>
            ...
            {["sales","leasing","both"].map(m=>{
              const META={sales:{icon:"🏷",title:"Sales Only",desc:"Leads · Property Builder · Pipeline · Discounts"},leasing:{icon:"🔑",title:"Leasing Only",desc:"Tenants · Contracts · Payments · Maintenance"},both:{icon:"◆",title:"Sales & Leasing",desc:"Full suite — teams fully segregated"}};
...
// ══════════════════════════════════════════════════════
const TABS=[
  {id:"dashboard", label:"Dashboard",        icon:"⊞",  group:"general", roles:["admin","sales_manager","sales_agent","leasing_manager","leasing_agent","viewer"]},
  {id:"leads",     label:"Leads",            icon:"👤", group:"sales",   roles:["admin","sales_manager","sales_agent"]},
  {id:"builder",   label:"Property Builder", icon:"🏗", group:"sales",   roles:["admin","sales_manager","sales_agent"]},
  {id:"pipeline",  label:"Pipeline",         icon:"⬡", group:"sales",   roles:["admin","sales_manager","sales_agent"]},
  {id:"leasing",   label:"Leasing",          icon:"🔑", group:"leasing", roles:["admin","leasing_manager","leasing_agent"]},
  {id:"discounts", label:"Discounts",        icon:"⚡", group:"shared",  roles:["admin","sales_manager","leasing_manager"]},
  {id:"activity",  label:"Activity Log",     icon:"📋", group:"shared",  roles:["admin","sales_manager","sales_agent","leasing_manager","leasing_agent"]},
  {id:"ai",        label:"AI Assistant",     icon:"✦",  group:"shared",  roles:["admin","sales_manager","sales_agent","leasing_manager","leasing_agent"]},
  {id:"users",     label:"Users",            icon:"👥", group:"general", roles:["admin"]},
];
const SUBTITLES={
  ...
  const MODES = [
    {
      ...
      tabs:["Leads","Property Builder","Pipeline","Discounts","AI Assistant"],
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0B1F3A 0%,#1A3558 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem"}}>
      <div className="fa" style={{background:"#fff",borderRadius:20,width:680,maxWidth:"100%",boxShadow:"0 30px 80px rgba(0,0,0,.4)",overflow:"hidden"}}>
        ...
        <div style={{background:"#0B1F3A",padding:"1.75rem 2rem"}}>
          ...
          <div style={{fontSize:14,color:"rgba(255,255,255,.6)",marginTop:4}}>First-time Setup — takes 2 minutes</div>
          {/* Progress */}
            ...
            <div className="fa">
              ...
              {mode==="both"&&(
                <div style={{background:"#E6EFF9",border:"1px solid #B5D4F4",borderRadius:10,padding:"12px 14px",marginBottom:18,fontSize:13,color:"#1A5FA8",lineHeight:1.7}}>
                  ...
                  • Sales staff (Sales Manager, Sales Agent) see only: Leads, Property Builder, Pipeline<br/>
...
export default function App(){
  ...
  const visibleTabs=TABS.filter(t=>t.roles.includes(userRole)&&allowedTabs.includes(t.id));

  if(!appConfig) return(<><GlobalStyle/><SetupWizard onComplete={cfg=>setAppConfig(cfg)}/></>);
  ...
  return(
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:"#F0F2F5",overflow:"hidden"}}>
      {/* Nav */}
      <div style={{background:"#0B1F3A",display:"flex",alignItems:"center",padding:"0 1rem",height:54,flexShrink:0,boxShadow:"0 2px 16px rgba(11,31,58,.5)",gap:0,overflowX:"auto"}}>
        {/* Logo */}
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:"#fff",fontWeight:700,marginRight:16,whiteSpace:"nowrap",flexShrink:0}}>
          <span style={{color:"#C9A84C"}}>◆</span> {appConfig?.company||"PropCRM"}
        </div>

        {/* Tab renderer helper */}
        {(()=>{
          const showGroups = cfg.mode==="both" && ["admin"].includes(userRole);
          const salesTabs   = visibleTabs.filter(t=>t.group==="sales");
          const leasingTabs = visibleTabs.filter(t=>t.group==="leasing");
          const generalTabs = visibleTabs.filter(t=>t.group==="general"&&t.id!=="users");
          const sharedTabs  = visibleTabs.filter(t=>t.group==="shared");
          const usersTabs   = visibleTabs.filter(t=>t.id==="users");

          const TabBtn = ({t}) => (
            <button key={t.id} onClick={()=>{setTab(t.id);if(t.id==="ai")loadAIData();}}
              style={{padding:"6px 11px",borderRadius:7,border:"none",background:tab===t.id?"rgba(201,168,76,.15)":"transparent",color:tab===t.id?"#C9A84C":"rgba(255,255,255,.6)",fontSize:12,fontWeight:tab===t.id?700:400,cursor:"pointer",display:"flex",alignItems:"center",gap:5,whiteSpace:"nowrap",transition:"all .15s",borderBottom:tab===t.id?"2px solid #C9A84C":"2px solid transparent",flexShrink:0}}>
              <span style={{fontSize:13}}>{t.icon}</span>{t.label}
            </button>
          );

          const Divider = () => (
            <div style={{width:1,height:26,background:"rgba(255,255,255,.12)",margin:"0 6px",flexShrink:0}}/>
          );

          const GroupLabel = ({label,color}) => (
            <div style={{fontSize:9,fontWeight:700,color,textTransform:"uppercase",letterSpacing:".8px",padding:"0 4px",flexShrink:0,marginRight:2}}>
              {label}
            </div>
          );

          if(!showGroups){
            // Simple flat nav for single-mode or non-admin
            return visibleTabs.map(t=><TabBtn key={t.id} t={t}/>);
          }

          // Grouped nav for Both-mode admin
          return (
            <>
              {/* General */}
              {generalTabs.map(t=><TabBtn key={t.id} t={t}/>)}
              ...
              {salesTabs.length>0&&(
                <>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:0,flexShrink:0}}>
                    <GroupLabel label="🏷 Sales" color="#4A9EE8"/>
                    <div style={{display:"flex",gap:0}}>
                      {salesTabs.map(t=><TabBtn key={t.id} t={t}/>)}
                    </div>
                  </div>
                </>
              )}
              ...
              {/* Leasing section */}
              {leasingTabs.length>0&&(
                <>
                  <Divider/>
                  <div style={{display:"flex",flexDirection:"column",gap:0,flexShrink:0}}>
                    <GroupLabel label="🔑 Leasing" color="#9B7FD4"/>
                    <div style={{display:"flex",gap:0}}>
                      {leasingTabs.map(t=><TabBtn key={t.id} t={t}/>)}
                    </div>
                  </div>
                </>
              )}
              ...
              {sharedTabs.length>0&&(
                <>
                  <Divider/>
                  {sharedTabs.map(t=><TabBtn key={t.id} t={t}/>)}
                </>
              )}
              ...
              {usersTabs.length>0&&(
                <>
                  <Divider/>
                  {usersTabs.map(t=><TabBtn key={t.id} t={t}/>)}
                </>
              )}
            </>
          );
        ...
        {/* Right side */}
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:10,flexShrink:0,paddingLeft:12}}>
          <div style={{textAlign:"right"}}>
