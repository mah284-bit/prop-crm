...
const ROLES = ["admin","sales_manager","sales_agent","leasing_manager","leasing_agent","viewer"];
...
const roleTeam = role => ({
  admin:"both", sales_manager:"sales", sales_agent:"sales",
...
function UserManagement({currentUser,leads,activities,showToast}){
  ...
  const blank={full_name:"",email:"",password:"",role:"sales_agent",is_active:true,end_date:"",department:"",phone:""};
  const[form,setForm]=useState(blank);
...
// ══════════════════════════════════════════════════════
const TABS=[
  {id:"dashboard", label:"Dashboard",        icon:"⊞", roles:["admin","sales_manager","sales_agent","leasing_manager","leasing_agent","viewer"]},
  {id:"leads",     label:"Leads",            icon:"👤", roles:["admin","sales_manager","sales_agent"]},
  {id:"builder",   label:"Property Builder", icon:"🏗", roles:["admin","sales_manager","sales_agent"]},
  {id:"leasing",   label:"Leasing",          icon:"🔑", roles:["admin","leasing_manager","leasing_agent"]},
  {id:"discounts", label:"Discounts",        icon:"⚡", roles:["admin","sales_manager","leasing_manager"]},
  {id:"pipeline",  label:"Pipeline",         icon:"⬡", roles:["admin","sales_manager","sales_agent"]},
  {id:"activity",  label:"Activity Log",     icon:"📋", roles:["admin","sales_manager","sales_agent","leasing_manager","leasing_agent"]},
  {id:"ai",        label:"AI Assistant",     icon:"✦", roles:["admin","sales_manager","sales_agent","leasing_manager","leasing_agent"]},
        ...
        <div style={{display:"flex",flexDirection:"column",flex:1,overflow:"hidden"}}>
          ...
          <div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:10,padding:"12px 14px",marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:8,marginBottom:10,alignItems:"center"}}>
              ...
              <button onClick={()=>{setFSearch("");setFProject("All");setFType("All");setFCat("All");setFStatus("All");setFPurpose("All");setFBeds("All");setFView("All");setFPriceMin("");setFPriceMax("");}} style={{padding:"7px 14px",borderRadius:8,border:"1.5px solid #E2E8F0",background:"#F0F2F5",color:"#4A5568",fontSize:12,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap"}}>✕ Reset</button>
...
// ══════════════════════════════════════════════════════════════════
// AI ASSISTANT — PropCRM Intelligence Layer
// Uses Claude API directly from browser
// ══════════════════════════════════════════════════════════════════
...
function buildContext(leads, units, projects, salePricing, leasePricing, activities, currentUser) {
  ...
  const pipeline = {};
  activeLeads.forEach(l => { pipeline[l.stage] = (pipeline[l.stage]||0)+1; });

  return `You are an AI assistant for PropCRM, a real estate CRM. The logged-in user is ${currentUser.full_name} (role: ${currentUser.role}).
...
- Active pipeline: ${activeLeads.length} leads
- My leads: ${myLeads.length}
- Pipeline breakdown: ${Object.entries(pipeline).map(([s,c])=>`${s}: ${c}`).join(", ")}
- Closed Won: ${leads.filter(l=>l.stage==="Closed Won").length}
- Closed Lost: ${leads.filter(l=>l.stage==="Closed Lost").length}

RECENT LEADS (last 10):
${leads.slice(0,10).map(l=>`- ${l.name} | Stage: ${l.stage} | Budget: AED ${Number(l.budget||0).toLocaleString()} | ${l.nationality||"Unknown"} | Source: ${l.source||"Unknown"} | Phone: ${l.phone||"—"} | Email: ${l.email||"—"}`).join("\n")}

PROPERTIES (${units.length} total units across ${projects.length} projects):
- Available: ${availUnits.length}
- Sold: ${units.filter(u=>u.status==="Sold").length}
- Reserved: ${units.filter(u=>u.status==="Reserved").length}
- Residential: ${units.filter(u=>u.unit_type==="Residential").length}
- Commercial: ${units.filter(u=>u.unit_type==="Commercial").length}

PROJECTS: ${projects.map(p=>`${p.name} (${p.developer||"—"}, ${p.status})`).join(", ")}

AVAILABLE UNITS SAMPLE (first 20):
${availUnits.slice(0,20).map(u=>{
...
RECENT ACTIVITIES: ${activities.slice(0,5).map(a=>`${a.type} with ${a.lead_name} by ${a.user_name}`).join(", ")}

=== YOUR CAPABILITIES ===
1. Answer questions about properties, leads, pipeline
2. Draft WhatsApp/email messages for leads
3. Suggest next actions for specific leads
4. Qualify leads and recommend stage moves
5. Auto-fill lead details from a description
6. Analyse pipeline and give business insights

When drafting messages, use professional Dubai real estate tone. Keep WhatsApp messages concise (under 150 words). For emails, be more detailed.
When suggesting stage moves, always check the requirements: Contacted needs phone+email, Site Visit needs meeting scheduled, Proposal needs unit linked+budget confirmed, Negotiation needs proposal notes, Closed Won needs final price+payment plan.
...
function AIAssistant({ leads, units, projects, salePricing, leasePricing, activities, currentUser, showToast }) {
  ...
  const QUICK_PROMPTS = [
    { icon:"📊", label:"Pipeline summary",        prompt:"Give me a summary of our current pipeline and suggest top 3 actions for this week." },
    { icon:"🏠", label:"Available units",          prompt:"List all available units under AED 3M with their key details." },
    { icon:"👤", label:"Leads needing attention",  prompt:"Which leads have been in the same stage the longest? Who should I contact today?" },
    { icon:"✉",  label:"Draft WhatsApp",           prompt:"Draft a professional WhatsApp follow-up message for a lead who went quiet after receiving a proposal." },
    { icon:"🔍", label:"Qualify a lead",           prompt:"I have a lead interested in a 2-bed apartment, budget AED 1.8M, UAE national, referred by a friend. What stage should they be in and what are the next steps?" },
    { icon:"📝", label:"Lead from description",    prompt:"Auto-fill: Ahmed Al Rashid, +971501234567, looking for a villa in Arabian Ranches, budget 5M, very serious buyer, met at a property exhibition." },
  ];
  ...
  useEffect(() => {
    setMessages([{
      role:"assistant",
      content:`Hello ${currentUser.full_name.split(" ")[0]}! 👋 I'm your PropCRM AI assistant. I have live access to your ${leads.length} leads, ${units.length} units across ${projects.length} projects, and recent activities.\n\nI can help you:\n• Find the right property for a client\n• Draft WhatsApp or email messages\n• Summarise your pipeline\n• Qualify leads and suggest next steps\n• Auto-create leads from a description\n\nWhat would you like to do?`
    }]);
  }, []);
  ...
  const sendMessage = async (text) => {
    ...
    try {
      const context = buildContext(leads, units, projects, salePricing, leasePricing, activities, currentUser);
      ...
      // Check if reply contains a lead suggestion
      if (msg.toLowerCase().includes("auto-fill") || msg.toLowerCase().includes("lead from")) {
        parseLead(reply);
      }
    } catch(e) {
      if (e.message.includes("401") || e.message.includes("invalid")) {
        setShowKeyInput(true);
        showToast("Invalid API key — please update it","error");
      } else {
        setMessages(p => [...p, { role:"assistant", content:`Sorry, I encountered an error: ${e.message}. Please try again.` }]);
      }
    }
    setLoading(false);
  };
  ...
  const parseLead = (text) => {
    // Try to extract lead details from AI response
    const nameMatch   = text.match(/name[:\s]+([A-Z][a-zA-Z\s]+?)(?:\n|,|\|)/i);
    const phoneMatch  = text.match(/(\+?971\d{8,9}|\+\d{10,14})/);
    const emailMatch  = text.match(/[\w.-]+@[\w.-]+\.\w{2,}/);
    const budgetMatch = text.match(/(?:budget|AED)[:\s]*([0-9,]+(?:\.[0-9]+)?(?:M|m|million)?)/i);

    if (nameMatch || phoneMatch) {
      let budget = 0;
      if (budgetMatch) {
        const raw = budgetMatch[1].replace(/,/g,"");
        budget = raw.toLowerCase().includes("m") ? parseFloat(raw)*1000000 : parseFloat(raw);
      }
      setSuggestion({
  ...
  const fmtMsg = (text) => {
    return text.split("\n").map((line, i) => {
      if (line.startsWith("•") || line.startsWith("-")) {
        return <div key={i} style={{paddingLeft:12,marginBottom:2}}>• {line.slice(1).trim()}</div>;
      }
      if (line.startsWith("**") && line.endsWith("**")) {
        return <div key={i} style={{fontWeight:700,marginBottom:4,marginTop:6}}>{line.slice(2,-2)}</div>;
      }
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",maxWidth:900,margin:"0 auto",width:"100%"}}>
      ...
      {/* API Key setup */}
      {showKeyInput && (
        <div style={{background:"#FDF3DC",border:"1.5px solid #E8C97A",borderRadius:10,padding:"14px 16px",marginBottom:14}}>
          <div style={{fontWeight:700,fontSize:14,color:"#8A6200",marginBottom:6}}>🔑 Claude API Key Required</div>
          <div style={{fontSize:13,color:"#4A5568",marginBottom:10,lineHeight:1.6}}>
            Get your free API key at <a href="https://console.anthropic.com" target="_blank" style={{color:"#1A5FA8"}}>console.anthropic.com</a> → API Keys → Create Key.
            It starts with <code>sk-ant-...</code>. Cost is roughly $0.003 per message.
          </div>
          <div style={{display:"flex",gap:8}}>
            <input
              type="password"
              ...
              id="api-key-input"
              style={{flex:1,padding:"9px 12px",border:"1.5px solid #D1D9E6",borderRadius:8,fontSize:13}}
            />
            <button onClick={()=>saveKey(document.getElementById("api-key-input").value.trim())}
              style={{padding:"9px 18px",borderRadius:8,border:"none",background:"#0B1F3A",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer"}}>
              Save Key
            </button>
            <button onClick={()=>setShowKeyInput(false)}
              style={{padding:"9px 14px",borderRadius:8,border:"1.5px solid #D1D9E6",background:"#fff",fontSize:13,cursor:"pointer"}}>
              Cancel
            </button>
          </div>
        </div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
        {QUICK_PROMPTS.map(p=>(
          <button key={p.label} onClick={()=>sendMessage(p.prompt)}
            style={{padding:"6px 12px",borderRadius:20,border:"1.5px solid #E2E8F0",background:"#fff",color:"#4A5568",fontSize:12,fontWeight:500,cursor:"pointer",display:"flex",alignItems:"center",gap:5,transition:"all .15s"}}
            onMouseOver={e=>{e.currentTarget.style.borderColor="#C9A84C";e.currentTarget.style.background="#FDF8EC";}}
            onMouseOut={e=>{e.currentTarget.style.borderColor="#E2E8F0";e.currentTarget.style.background="#fff";}}>
            {p.icon} {p.label}
          </button>
        ))}
        <button onClick={()=>setShowKeyInput(s=>!s)}
          style={{padding:"6px 12px",borderRadius:20,border:"1.5px solid #E2E8F0",background:"#fff",color:"#A0AEC0",fontSize:12,cursor:"pointer",marginLeft:"auto"}}>
      ...
      <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:12,paddingRight:4,marginBottom:12}}>
        {messages.map((m,i)=>(
          <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start",flexDirection:m.role==="user"?"row-reverse":"row"}}>
            <div style={{width:32,height:32,borderRadius:"50%",background:m.role==="user"?"#0B1F3A":"#C9A84C",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>
              {m.role==="user"?"👤":"✦"}
            </div>
            <div style={{maxWidth:"75%",background:m.role==="user"?"#0B1F3A":"#fff",color:m.role==="user"?"#fff":"#1a2535",borderRadius:m.role==="user"?"12px 12px 4px 12px":"12px 12px 12px 4px",padding:"10px 14px",fontSize:13,lineHeight:1.7,border:m.role==="assistant"?"1px solid #E2E8F0":"none",boxShadow:m.role==="assistant"?"0 1px 4px rgba(0,0,0,.06)":"none"}}>
        ...
        {loading && (
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            <div style={{width:32,height:32,borderRadius:"50%",background:"#C9A84C",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>✦</div>
            <div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:"12px 12px 12px 4px",padding:"10px 16px",display:"flex",gap:6,alignItems:"center"}}>
              <div style={{width:8,height:8,borderRadius:"50%",background:"#C9A84C",animation:"pulse 1s infinite"}}/>
              <div style={{width:8,height:8,borderRadius:"50%",background:"#C9A84C",animation:"pulse 1s infinite .2s"}}/>
              <div style={{width:8,height:8,borderRadius:"50%",background:"#C9A84C",animation:"pulse 1s infinite .4s"}}/>
              <style>{`@keyframes pulse{0%,100%{opacity:.3}50%{opacity:1}}`}</style>
            </div>
          </div>
        )}

        {/* Lead suggestion card */}
        {suggestion && (
          <div style={{background:"#E6F4EE",border:"1.5px solid #A8D5BE",borderRadius:12,padding:"12px 16px",margin:"8px 0"}}>
            <div style={{fontWeight:700,fontSize:13,color:"#1A7F5A",marginBottom:8}}>✓ Lead detected — add to PropCRM?</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,fontSize:12,color:"#4A5568",marginBottom:10}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={async()=>{
                try{
                  const{data,error}=await db.from("leads").insert({...suggestion,created_by:currentUser.id,stage_updated_at:new Date().toISOString()}).select().single();
                  if(error)throw error;
                  showToast(`${suggestion.name} added as lead!`,"success");
                  setSuggestion(null);
                }catch(e){showToast(e.message,"error");}
              }} style={{padding:"7px 16px",borderRadius:8,border:"none",background:"#1A7F5A",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer"}}>
                + Add Lead
              </button>
              <button onClick={()=>setSuggestion(null)} style={{padding:"7px 12px",borderRadius:8,border:"1.5px solid #A8D5BE",background:"#fff",fontSize:12,cursor:"pointer"}}>
                Dismiss
              </button>
            </div>
          </div>
      ...
      {/* Input bar */}
      <div style={{display:"flex",gap:8,background:"#fff",border:"1.5px solid #E2E8F0",borderRadius:12,padding:"8px 8px 8px 14px",boxShadow:"0 2px 8px rgba(0,0,0,.06)"}}>
        <textarea
          value={input}
          onChange={e=>setInput(e.target.value)}
          onKeyDown={e=>{ if(e.key==="Enter"&&!e.shiftKey){ e.preventDefault(); sendMessage(); }}}
          placeholder="Ask anything… 'Show 2-bed units under AED 2M', 'Draft WhatsApp for Ahmed', 'Summarise pipeline'"
          style={{flex:1,border:"none",outline:"none",resize:"none",fontSize:13,lineHeight:1.5,minHeight:44,maxHeight:120,fontFamily:"inherit",background:"transparent"}}
          rows={1}
        />
        <button onClick={()=>sendMessage()} disabled={loading||!input.trim()}
          style={{padding:"10px 18px",borderRadius:8,border:"none",background:loading||!input.trim()?"#E2E8F0":"#0B1F3A",color:loading||!input.trim()?"#A0AEC0":"#fff",fontSize:13,fontWeight:600,cursor:loading||!input.trim()?"not-allowed":"pointer",transition:"all .15s",alignSelf:"flex-end"}}>
          {loading?"…":"Send ↑"}
        </button>
      </div>
