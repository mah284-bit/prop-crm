...
const MODE_TABS = {
  sales:   ["dashboard","leads","builder","discounts","pipeline","activity","ai","users"],
  leasing: ["dashboard","leasing","discounts","activity","ai","users"],
  both:    ["dashboard","leads","builder","leasing","discounts","pipeline","activity","ai","users"],
};
      ...
      {subTab==="settings"&&(
        <div className="fade-in" style={{flex:1,overflowY:"auto"}}>
          <div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:14,padding:"1.5rem",marginBottom:16}}>
            ...
            <div style={{fontSize:13,color:"#718096",marginBottom:18}}>Change which modules are visible across the entire application. Takes effect immediately for all users.</div>
            {["sales","leasing","both"].map(m=>{
              const META={sales:{icon:"🏷",title:"Sales Only",desc:"Leads · Property Builder · Pipeline · Discounts"},leasing:{icon:"🔑",title:"Leasing Only",desc:"Tenants · Contracts · Payments · Maintenance"},both:{icon:"◆",title:"Sales & Leasing",desc:"Full suite — both teams with role segregation"}};
              const isCur=(appConfig?.mode||"both")===m;
              return(
                <div key={m} onClick={()=>{if(!isCur&&onConfigChange){onConfigChange({...(appConfig||{}),mode:m});showToast(`Switched to ${META[m].title} mode`,"success");}}}
                  ...
                  <div style={{flex:1}}>
                    <div style={{fontWeight:700,fontSize:14,color:"#0B1F3A"}}>{META[m].title} {isCur&&<span style={{fontSize:11,color:"#1A7F5A",fontWeight:700}}>✓ CURRENT</span>}</div>
          ...
          <div style={{background:"#fff",border:"1px solid #E2E8F0",borderRadius:14,padding:"1.5rem"}}>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:17,fontWeight:700,color:"#0B1F3A",marginBottom:4}}>Company Details</div>
            <div style={{fontSize:13,color:"#718096",marginBottom:14}}>Used in AI assistant context and throughout the app.</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {[["Company Name","company","text","Al Mansoori Real Estate"],["Currency","currency","select",""],["Country","country","select",""]].map(([label,field,type,ph])=>(
                <div key={field} style={{gridColumn:field==="company"?"1/-1":"auto"}}>
                  <label style={{fontSize:11,fontWeight:600,color:"#4A5568",display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:".5px"}}>{label}</label>
                  {type==="select"?(
                    <select defaultValue={appConfig?.[field]||""} id={`cfg-${field}`}
                      style={{width:"100%",padding:"9px 12px",border:"1.5px solid #D1D9E6",borderRadius:8,fontSize:13}}>
                      {field==="currency"?[["AED","AED — UAE Dirham"],["SAR","SAR — Saudi Riyal"],["USD","USD — US Dollar"],["GBP","GBP — British Pound"]].map(([v,l])=><option key={v} value={v}>{l}</option>):["UAE","Saudi Arabia","UK","USA","India","Other"].map(c=><option key={c} value={c}>{c}</option>)}
                    </select>
                  ):(
                    <input type="text" id={`cfg-${field}`} defaultValue={appConfig?.[field]||""}
                      placeholder={ph} style={{width:"100%",padding:"9px 12px",border:"1.5px solid #D1D9E6",borderRadius:8,fontSize:13}}/>
                  )}
                </div>
              ))}
            </div>
