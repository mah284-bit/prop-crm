...
function LoginScreen({ onLogin }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!profile) throw new Error("Profile not found. Contact your admin.");
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0B1F3A 0%,#1A3558 60%,#0B1F3A 100%)", display:"flex", alignItems:"center", justifyContent:"center", padding:"1rem" }}>
      <div className="fade-in" style={{ background:"#fff", borderRadius:20, padding:"2.5rem", width:420, maxWidth:"100%", boxShadow:"0 30px 80px rgba(0,0,0,0.4)" }}>
        {/* Logo */}
        <div style={{ textAlign:"center", marginBottom:32 }}>
          ...
          <div style={{ fontSize:13, color:"#A0AEC0", marginTop:6 }}>Sign in to your account</div>
        </div>

        {error && (
          <div style={{ background:"#FAEAEA", color:"#B83232", border:"1.5px solid #F0BCBC", borderRadius:8, padding:"10px 14px", fontSize:13, marginBottom:16 }}>
            {error}
          </div>
        )}
        ...
        <Btn onClick={submit} disabled={loading} full style={{ marginTop:8, padding:"12px" }}>
          {loading ? "Signing in…" : "Sign In →"}
        </Btn>

        <div style={{ marginTop:24, padding:"14px", background:"#F7F9FC", borderRadius:10, border:"1px solid #E2E8F0" }}>
          <div style={{ fontSize:11, fontWeight:600, color:"#A0AEC0", textTransform:"uppercase", letterSpacing:"0.5px", marginBottom:8 }}>First time setup?</div>
          <div style={{ fontSize:12, color:"#4A5568", lineHeight:1.7 }}>
            Your admin needs to create your account in the <strong>User Management</strong> panel. Contact them to get your login credentials.
          </div>
        </div>
      </div>
    </div>
...
function UserManagement({ currentUser, showToast }) {
  ...
  const [form, setForm] = useState({ full_name:"", email:"", password:"", role:"agent", is_active:true });
  ...
  const blank = { full_name:"", email:"", password:"", role:"agent", is_active:true };

  const createUser = async () => {
    if (!form.full_name||!form.email||!form.password) { showToast("Please fill in all required fields.", "error"); return; }
    try {
      // Create auth user via admin API (requires service role — use edge function in prod)
      // For simplicity we use signUp which sends a confirmation email
      ...
      showToast(`User ${form.full_name} created. They will receive a confirmation email.`, "success");
  ...
  const updateUser = async () => {
    ...
    try {
      ...
      setEditUser(null); setForm(blank); loadUsers();
    } catch(e) { showToast(e.message, "error"); }
  };

  const toggleActive = async (user) => {
    if (user.id === currentUser.id) { showToast("You cannot deactivate yourself.", "error"); return; }
    const { error } = await supabase.from("profiles").update({ is_active: !user.is_active }).eq("id", user.id);
    if (!error) { showToast(`${user.full_name} ${user.is_active?"deactivated":"reactivated"}.`, "success"); loadUsers(); }
  };

  const openEdit = (u) => { setEditUser(u); setForm({ full_name:u.full_name, email:u.email, password:"", role:u.role, is_active:u.is_active }); };
  ...
  return (
    <div className="fade-in" style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:16 }}>
        {[["Total Users",stats.total,"#0B1F3A"],["Active",stats.active,"#1A7F5A"],["Admins",stats.admins,"#8A6200"],["Agents",stats.agents,"#1A5FA8"]].map(([l,v,c])=>(
          <div key={l} style={{ background:"#fff", border:"1px solid #E2E8F0", borderRadius:10, padding:"12px 16px", borderTop:`3px solid ${c}` }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        <div style={{ fontSize:13, color:"#A0AEC0" }}>{users.length} user{users.length!==1?"s":""} registered</div>
      ...
      {loading ? <Spinner/> : (
        <div style={{ flex:1, overflowY:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead>
              <tr style={{ background:"#0B1F3A" }}>
                {["User","Email","Role","Status","Last Sign-in","Actions"].map(h=>(
                  ...
                  <td style={{ padding:"12px 14px" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      ...
                      <div>
                        ...
                        {u.id===currentUser.id && <div style={{ fontSize:10, color:"#C9A84C", fontWeight:600 }}>YOU</div>}
                      </div>
                    </div>
                  </td>
                  <td style={{ padding:"12px 14px", fontSize:13, color:"#4A5568" }}>{u.email}</td>
                  <td style={{ padding:"12px 14px" }}><RoleBadge role={u.role}/></td>
                  <td style={{ padding:"12px 14px" }}>
                    <span style={{ fontSize:11, fontWeight:600, padding:"3px 9px", borderRadius:20, background:u.is_active?"#E6F4EE":"#F0F2F5", color:u.is_active?"#1A7F5A":"#A0AEC0" }}>
                      {u.is_active?"Active":"Inactive"}
                    </span>
                  </td>
                  <td style={{ padding:"12px 14px", fontSize:12, color:"#A0AEC0" }}>{fmtDate(u.last_sign_in_at)}</td>
                  <td style={{ padding:"12px 14px" }}>
                    <div style={{ display:"flex", gap:6 }}>
                      <Btn small variant="outline" onClick={()=>openEdit(u)}>Edit</Btn>
                      {u.id!==currentUser.id && (
                        <Btn small variant={u.is_active?"danger":"green"} onClick={()=>toggleActive(u)}>
          ...
          {users.length===0 && <Empty icon="👥" msg="No users yet — add your first user"/>}
        </div>
      )}
      ...
      {showAdd && (
        <Modal title="Add New User" onClose={()=>setShowAdd(false)}>
          <div style={{ background:"#E6EFF9", borderRadius:8, padding:"10px 14px", marginBottom:16, fontSize:13, color:"#1A5FA8" }}>
            ℹ The new user will receive a confirmation email and must verify their address before logging in.
          </div>
          <G2>
          ...
          <FF label="Email Address" required><input type="email" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} placeholder="sara@yourcompany.com"/></FF>
          ...
          <div style={{ background:"#F7F9FC", borderRadius:8, padding:"10px 14px", marginBottom:16, fontSize:12, color:"#4A5568" }}>
            <strong>Role permissions:</strong><br/>
            <strong>Admin</strong> — full access, user management, see all leads<br/>
            <strong>Manager</strong> — see all leads, edit/delete, no user management<br/>
            <strong>Agent</strong> — see and manage own leads only<br/>
      ...
      {/* Edit User Modal */}
      {editUser && (
        <Modal title={`Edit User — ${editUser.full_name}`} onClose={()=>setEditUser(null)} width={420}>
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session?.user) {
        ...
        if (profile?.is_active) setCurrentUser({ ...session.user, ...profile });
      }
      setChecking(false);
    });
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === "SIGNED_OUT") { setCurrentUser(null); setLeads([]); setProperties([]); setActivities([]); }
    });
      ...
      <div style={{ display:"flex", flexDirection:"column", height:"100vh", background:"#F0F2F5", overflow:"hidden" }}>
        ...
        <div style={{ flex:1, overflow:"hidden", padding:"0 1.5rem 1.5rem" }}>
          {dataLoading ? <Spinner msg="Loading your data…"/> : (<>
            ...
            {tab==="users"      && can(currentUser.role,"manage_users") && <UserManagement currentUser={currentUser} showToast={showToast}/>}
          </>)}
        </div>
      </div>
